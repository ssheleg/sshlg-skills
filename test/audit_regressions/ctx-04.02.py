#!/usr/bin/env python3
"""CTX-04.02 — actual-load acceptance for Claude Code (sherlock audit, CTX-04).

The decision (CTX-04.01) names Claude Code a MANDATORY host. This is its
...
docs/evidence/acceptance/ctx-04.02.json; the plain run verifies it against a
...
RECEIPT = os.path.join(ROOT, "docs", "evidence", "acceptance", "ctx-04.02.json")
...
# testing a copy, not the thing Claude Code actually loads.
_PROBE_JS = r'''
const fs = require('fs');
const os = require('os');
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ctx0402-'));
const claudeT = apply.TARGETS.filter((t) => t.agent === 'claude');
// clean session: a fresh Claude Code host with an installed-but-empty routing block
const cleanHome = path.join(tmp, 'clean');
fs.mkdirSync(path.join(cleanHome, '.claude'), { recursive: true });
const cleanFile = path.join(cleanHome, '.claude', 'CLAUDE.md');
fs.writeFileSync(cleanFile, '# CLAUDE.md\n\noperator content\n\n' + apply.EMPTY_BLOCK + '\n');
const r1 = apply.apply({ home: cleanHome, hostTargets: claudeT, routers, includeCursor: false, log: () => {} });
const clean = loadedBlock(cleanFile);
// upgraded session: re-apply into the same host (idempotent load)
const r2 = apply.apply({ home: cleanHome, hostTargets: claudeT, routers, includeCursor: false, log: () => {} });
const upgraded = loadedBlock(cleanFile);
const preserved = fs.readFileSync(cleanFile, 'utf8').includes('operator content');
// degraded: no .claude dir -> agent-absent (NOT_RUN, no workaround)
const bareHome = path.join(tmp, 'bare');
fs.mkdirSync(bareHome, { recursive: true });
const r3 = apply.apply({ home: bareHome, hostTargets: claudeT, routers, includeCursor: false, log: () => {} });
// degraded: unsupported platform -> platform-unsupported (NOT_RUN)
const r4 = apply.apply({ home: cleanHome, platform: 'sunos', hostTargets: claudeT, routers, includeCursor: false, log: () => {} });
// native corroboration: the block ACTUALLY present in the live ~/.claude
const liveFile = path.join(os.homedir(), '.claude', 'CLAUDE.md');
const live = fs.existsSync(liveFile) ? loadedBlock(liveFile) : null;
fs.rmSync(tmp, { recursive: true, force: true });
console.log(JSON.stringify({
  clean, upgraded, preserved,
  clean_action: (r1.targets.find((t) => t.agent === 'claude') || {}).action,
  upgraded_action: (r2.targets.find((t) => t.agent === 'claude') || {}).action,
...
def build_receipt():
    ...
    return {
        "id": "CTX-04.02",
        "acceptance": "actual-load: Claude Code (mandatory host) via the supported adapter",
        "generated_by": "test/audit_regressions/ctx-04.02.py --emit",
        ...
        "notes": "loaded bytes/digest/namespaces are read from an ACTUAL apply through the "
                 "Claude Code adapter; an absent host or unsupported platform is NOT_RUN with "
...
def load_receipt():
    assert os.path.isfile(RECEIPT), \
        "no receipt — run `python3 test/audit_regressions/ctx-04.02.py --emit`"
