...
|---|---|---|
| `SessionStart` | one note saying the block is not advisory; the session's title and the ledger to watch; **and, since v0.46.0, one line naming any other enabled plugin that also injects here** — silent when there is none | ~90 tokens per session |
