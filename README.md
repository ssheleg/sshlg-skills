...
| **[agent-stack](https://github.com/ssheleg/agent-stack)** | 0.6.1 | Production patterns for agent orchestrators — tool-calling loops that survive their own context pressure, pipelines with human checkpoints and resume, provider routing with fallback and health checks, four memory layers with confidence decay and conflict resolution; **eval suites that measure whether the agent actually works** — judging output, regression gates, scoring a run; plus the wallet side of reselling LLM access: tiered balances, one markup boundary, two-phase commit against a provider API. |
