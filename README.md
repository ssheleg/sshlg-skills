...
|---|---|---|
| **[super-ux](https://github.com/ssheleg/super-ux)** | 0.47.0 | Scenario-driven UI development. A versioned design chain in `docs/ux/` — the product vision → personas and jobs → user flows → a screens-and-states map with Figma frames → traced scenarios → evidence-backed audits → fix plans, plus `docs/brand/` for how the product speaks. One `/ux` entry point that reaches every skill, two doc-drift linters and a contract doctor. |
...
|---|---|---|
| `SessionStart` | one note saying the block is not advisory; the session's title and the ledger to watch; **and, since v0.47.0, one line naming any other enabled plugin that also injects here** — silent when there is none | ~90 tokens per session |
