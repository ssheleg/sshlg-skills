...
|---|---|---|---|---|---|
| RT-1 | The version is readable in the copy that executes | `lib/updaterun.js` opened with `require('../package.json')`; the wired runtime carries `hooks/`, `lib/` and `skills.json` and no manifest, so the module threw at load and the unattended update shipped in v1.48.0 never ran once. Scanned against the runtime the hooks were actually wired to: 21 relative requires, 1 dangling, and the one was the new feature. The scope is wider than the unattended run: THREE entry points read that manifest — `hooks/session-start.js` for the notice and the probe, `hooks/notification.js` for the idle spawn, `updaterun.js` for the version reached — so the whole surface was inert. Receipt: `~/.sshlg-skills/state.json` holds `inbound`, written by the handoff block of the SAME hook in the SAME release, and holds no `updateCheck` key at all. `package.json` is in the managed set now, so the version read is the generation whose modules are running | v1.48.2 | a future module reaching outside `hooks/` and `lib/`, which is what RT-3 refuses | 2026-09-10, main thread, against `~/.sshlg-skills/runtime` |
| RT-6 | The surface is confirmed by EXECUTION in the wired runtime, which is what RT-5 says is the only reading that counts | after `npx --yes sshlg-skills@latest update` installed v1.48.2 (`refreshed 54 file(s) — 1 new, 4 changed`, the new one `+ package.json`), the live runtime scanned 21 relative requires and 0 dangling — the same tree that reported 1. Then the wired `hooks/session-start.js` was RUN with a real payload: exit 0, and `~/.sshlg-skills/state.json` gained `updateCheck` = `{"at": 1789075598279, "latest": "1.48.2"}` within four seconds. That key had never existed on this machine across every session since v1.48.0. The notice itself correctly printed nothing, because installed equals latest | v1.48.2 | reading a green `update` run as proof — it reported 55 of 55 green while the surface was dead, because copying files is what it checks | 2026-09-10, main thread, on the wired runtime |
| RT-7 | Ten minutes was the right size, and the number is measured rather than argued | the first release through the fixed step was this one. `OK: registry serves sshlg-skills@1.48.2 (after 330s)` — five and a half minutes, so the three-minute window this replaced would have failed v1.48.2 exactly as it failed v1.48.1. Two consecutive releases past the old bound is the evidence for the new one | v1.48.2 | a future edit shortening the window, which now has two dated counter-examples | 2026-09-10, GitHub Actions run 34530775197 |
| RT-5 | Presence in the runtime was checked; execution in it was not | v1.48.0's own row SU-1 says the surface was "exercised live" — and it was, against the package, in-process and through e2e fixtures that spawn the scripts from the checkout. The post-release check on this machine confirmed both new modules were PRESENT in `~/.sshlg-skills/runtime/lib/`. Both readings were true and neither could see the defect, because the file that was missing is one the package has and the runtime never copied. What closes the gap is not another reading but RT-3, which exercises a SYNCED runtime rather than the tree the tests run from | v1.48.2 | a future release confirmed by listing files in the runtime, which is what this row exists to stop | 2026-09-10, main thread |
| RT-2 | Unknown is recorded as unknown, not as unchanged | the old fallback set `to = from` when the version was unreadable — indistinguishable from "the set did not move", and `ranLine` is silent on that, so an update that DID move the set told the operator nothing and the session kept running the copy it started with. `to` is `null` now, and a succeeded run with an unreadable version says so AND asks for the restart. Two fixtures: the new branch, and one asserting the unknown and unchanged lines are not equal | v1.48.2 | collapsing the two records again, which reads as a tidier API and loses the only signal the operator acts on | 2026-09-10, main thread |
| RT-3 | The class is guarded, not the instance | `test/runtime_closure_test.js` syncs the package into a temp runtime and refuses any relative `require` that does not resolve inside it. Watched failing with the fix reverted, naming `lib/updaterun.js -> ../package.json` and nothing else. Five further cases: the corpus is asserted substantial (≥20 modules, ≥15 requires) because a guard that examines nothing passes everything; a planted reach-out is caught; the copied manifest must carry no `type` field; a second sync copies nothing | v1.48.2 | the check being narrowed to `package.json`, which would make it a fixture for one bug rather than a rule about the runtime | 2026-09-10, main thread |
| RT-4 | The guard reads what would RUN, not what its own comments contain | the first draft reported `lib/runtime.js` as dangling on the strength of a sentence in its header explaining this bug — the defect `lib/hygiene.js` had already paid for once. It strips comments and KEEPS string literals, and one fixture holds both halves in one file: the commented requires must NOT be reported and a real one beside them must be, because a scanner reporting neither would pass a test that only asserted the comment is ignored | v1.48.2 | a needle-style rewrite of the scanner that goes back to raw text | 2026-09-10, main thread |
...
| PRG-4 | A failure summary exists, and it says what to do next | `FAILED 18 of 18 steps` followed by the exact command per line and a re-run instruction. Before this the exit code was the ONLY signal that one of 55 steps broke, and finding it meant scrolling back through 55 spawns | v1.23.0 | the summary listing failures without the re-run line — asserted separately for that reason | 2026-09-01, main thread |
...
| PP-3a | The agent-stack pin moves to 0.18.1 on all three surfaces | `git submodule status` → `skills/agent-stack (v0.18.1)`; `skills.json` and the README row both read 0.18.1; `npm test` → rc=0, `COUNTED: 45 suites, 768 fixtures, 9 pinned members` | v1.12.1 | any agent-stack release after 2026-08-31 | verified |
| PP-4a | This validator refused a member's malformed board row that the member's own gate passed | the first re-pin attempt failed with *"row B-124 has 12 cells against the 3 its own header declares"* while `agent-stack`'s `npm test` was green on the same tree; the member shipped 0.18.1 to repair it, and the pin advanced only after. **The refusal is the evidence** — a member's board is checked here, not there | v1.12.1 | never | verified |
...
| PT4-1 | The `task-pipeline` pin is the version the registry actually serves | `npm view task-pipeline-skill version` → 1.82.4 at cut time; the submodule sits on annotated `v1.82.4` and `git -C skills/task-pipeline describe --tags` agrees; the three pin surfaces — `skills.json`, the README row, the submodule pointer — moved together | v1.28.0 | any task-pipeline release after 2026-09-03 | verified |
| PT4-2 | `v1.82.4` was cut against a tree that had already passed everything the release runs | tag made locally on `7f32d0a`, `npm run test:all` run in a worktree at the tag → rc=0 read from the suite, 13 stages, `PASS: all 424 guards provably reject their planted defect`; pushed only then; release run 33687844194 success, `npm view` 1.82.4, and the workflow's own *The registry must actually serve it* step green | v1.28.0 | a release cut without that worktree run | verified |
| PT4-3 | The ratchet sentence is now derived from the run, not from the marker it quotes | `test/run.js` `proseRatchets()` parses `**Ratchets.** N suites, M fixtures, K pinned members` and compares against the measured triple; planted `fixtures=1` into a copy → `FAIL: docs/DOCMAP.md's Ratchets sentence says fixtures=1, this run counted 799`; sentence deleted → `FAIL: … has no `**Ratchets.**` sentence`. Both watched refusing before shipping | v1.28.0 | the sentence being reworded past the pattern | verified |
| PT4-4 | The whole gate is green at the re-derived ratchet | `npm test` exit 0 read explicitly, not from trailing output: `COUNTED: 46 suites, 799 fixtures, 9 pinned members`; the marker and its sentence now both carry 799, and the README's negative-self-test count moved 28 → 29 because `validate.py` refused the tree until it was recomputed | v1.28.0 | the next suite or fixture added | verified |

## 2026-09-03 — v1.29.0, ten board rows and three defects the closes found

| id | What | Evidence | Shipped in | Invalidated by | Human |
|---|---|---|---|---|---|
| CL-1 | The open count is read by the file's own published recipe, not by a grep | the first reading said 19 and the recipe gives 21: status is the second-to-last pipe-delimited field, which a naive split reads as the priority column. 21 → 11 after this pass, recounted by the same recipe | v1.29.0 | the board's column order changing | verified |
| CL-2 | `B-92` asks the pin question and asks it first | `test/validate.py` resolves `git ls-files -s <dir>` and requires a ref under `refs/remotes` to contain it; planted pin at an unpushed commit → `the PIN names 370fbac and no remote ref contains it`; control, same local commit with the pointer untouched → `PASS: sshlg-skills structure valid`. The ordering bug was found BY the plant: after the upstream question the check was skipped for a branch with no upstream | v1.29.0 | the guard being reordered behind another early return | verified |
| CL-3 | `B-106` discloses unknown ownership and reads the subshell form | `node test/repogate_test.js` → `OK (18 checks)`; `unresolvedCd('(cd $n && git commit -m x)')` true, `('(cd skills/x && …)')` false; `commitDirs('(cd skills/x && git commit -m x)')` → `['skills/x']`, which was `['.']` before — the umbrella claiming a member's commit on a literal path | v1.29.0 | a new quoting form the segmenter does not split | verified |
| CL-4 | `B-103`'s convention is declared and checked, and three live collisions were repaired | `check_ledger_ids_are_unique_within_their_section` refuses a duplicated `PP-1` at the line of the first; `PP-2`, `PP-3`, `PP-4` disambiguated with the trailing-letter form; the rename exposed a counter whose grammar excluded that form and had under-reported by seven — 576/502 stated against 583/509 measured | v1.29.0 | a fourth id form the grammar does not cover | verified |
| CL-5 | `B-107`'s queue has a negative control, not only a test | both original defects replanted in a copy: status read from the row body → 1 failure; the closed-section flag latching → 2 failures. `python3 test/queue_test.py` → `Ran 8 tests OK` on the real tree | v1.29.0 | `scan()` being inlined back into the script body | verified |
| CL-6 | `B-91`'s convergence check was watched in all three states | empty range → `dormant: no component pointer moved in this range`, rc=0; `1c23d56..HEAD` with no record → **rc=1**, naming `skills/task-pipeline`; with the record → rc=0, `VERDICT: PASS — components 9 · live checks 12`. The record names the pinned sha because the checker resolves the pointer, not the tag | v1.29.0 | a component being added without a record convention | verified |
| CL-7 | `B-75`'s git lease round-trips against the real remote | `acquire GITPROBE` → `exclusive across machines`; `git ls-remote origin 'refs/agent-sync/*'` → 1 ref; `release` → 0 refs. The first attempt wrote `backend`, a key this tool does not read, and its own status line kept advising the change it had been given | v1.29.0 | agent-sync renaming the key | verified |
...
| CL-12 | `B-76`'s conflict does not exist in the family as it stands | `skills/task-pipeline/test/validate.py:90` fails on `not desc.startswith("Use when ")` — it demands the opener the row says it refuses — and asserts the capability half on the next line; `audit_skill.py --house` over every shipped skill gives **DESC_USEWHEN pass 28, gap 0** | v1.30.0 | either checker changing its opener rule | verified |
| CL-13 | The WHAT half is unchecked family-wide, and the obvious prototype for it does not work | nine `DESC_*` rules in `audit_skill.py`, none about capability; a regex prototype that strips the trigger clause, trigger list and refusal reported a 0-character WHAT half for six skills and missed the opening clause of twenty, because several descriptions are YAML block scalars. Filed as `B-139` with the numbers rather than shipped as a heuristic | v1.30.0 | a capability rule landing in the shared auditor | verified |

## 2026-09-03 — v1.30.0, the role contradiction and the empty LEGACY_FIT

| id | What | Evidence | Shipped in | Invalidated by | Human |
|---|---|---|---|---|---|
| RC-1 | No member's role answers a question another router owns | `test/routers_test.js` compares every `skills.json` role against every router's `answers`, with `seo-aeo-audit`/`seo-llmo` declared as the one exemption; replanting the old cell gives `FAIL: super-ux's role answers copywriting's question: "how it sounds"`, `1 failure(s) out of 38 checks` | v1.30.0 | a router's `answers` being reworded to overlap a role | verified |
| RC-2 | `LEGACY_FIT` is empty and every member commits the umbrella's exact pixels | `node test/site_test.js` → `PASS: site — 42 checks (13 pages, 9 members, 12 routers)`; cards 8470→8378, 8167→7974, 8966→8819 bytes, copied from `og/skills-<m>.png` after the set was emptied so the corrected metric is what rendered them | v1.30.0 | a member regenerating its card locally again | verified |
| RC-3 | The three pins are the versions the registry serves | `npm view` at cut time: `super-ux` 0.52.5, `sheleg-design-skill` 1.58.4, `@ssheleg/sheleg-dev` 0.11.5; each submodule on its annotated tag, `skills.json` and the README row moved with it | v1.30.0 | any of the three releasing again | verified |
...
| LW-1 | The director's mandated command has a fallback, as that skill's front matter promises for every optional sibling | `sheleg-design` 1.59.1, Act 2: the lane table and the harness's own skill list cover *whose answer* and *what do I have*; *what is missing* and *what was declined* are named as unrecoverable and the cast must say the roster was unmeasured | v1.32.1 | the fallback being dropped in a later edit | verified |
| LW-2 | A `provides` id is one only its own entry could have put there | `design-system` excluded from `ui-ux-pro-max`, with the reason in `lib/packs.js` at the declaration: it is also shipped by `anthropics/knowledge-work-plugins`, so listing it would report `ui-ux-pro-max` present on a machine that has never seen it | v1.32.1 | a shared id being added to any `provides` | verified |
| LW-3 | Both findings came from the ladder walk, not from a gate | the member validator holds the companion, its link, its mirror and its contents list and has no rule about a sibling's fallback; `npm test` was green on both repositories before either was found | v1.32.1 | a mechanical check taking over either claim | verified |
| RT-1 | The pack's declared addresses are checked on every push | `.github/workflows/validate.yml` runs `pack design --check` beside `check_pins.py`, warn-only; `docs/DOCMAP.md`'s propagation row names the CI half beside the on-add half | v1.32.2 | the step being removed or made blocking | verified |
| RT-2 | `--check` was watched firing against a live rename, not a synthetic one | planting `accesslint/claude-marketplace` (which still 301s to `AccessLint/skills`) gave `MOVED → AccessLint/skills`, `1 address(es) need editing`, exit **2**; the corrected address gives exit **0** | v1.32.2 | GitHub dropping the redirect | verified |
| RT-3 | The code is never 1, on any input | `test/packs_test.js` asserts `checkExit` over six shapes — clean, ok, moved, 404, archived, unreachable, and the empty list — and refuses 1 for all of them | v1.32.2 | a blocking code being introduced | verified |
| RT-4 | An offline run cannot turn the build red | an `error` row increments `unreachable` and not `bad`; `checkExit` returns 0 and the report still prints `Every declared address resolves to itself` | v1.32.2 | `unreachable` being counted as a defect | verified |
| RV-1 | The block's own design vocabulary reaches a route | `node test/route_coverage.js` → 81 of 99, from 74 before; `редизайн сайта`, `сверстай лендинг`, `свёрстай дашборд`, `нужна вёрстка дашборда` and `redesign the landing page` all name `sheleg-design`, and all five reached nothing at v1.32.2 | v1.33.0 | the member dropping any of the four advertised words | verified |
| RV-2 | A trigger carrying ё routes identically without it | `normalise()` and `phrasePattern` fold; `звезды телеграм` → `telegram-dev` and `отчет о приемке` → `evidence-docs`, both nothing before. `test/triggers_test.js` walks every ё-carrying trigger in the shipped table and compares both spellings | v1.33.0 | a caller normalising text before `matches()` differently | verified |
...
| IS-1 | A shortlist can be empty, and one shared word is not a hit | `test/toolkit_test.js`: a query no description carries prints *NOTHING on this machine matched*; `rank(SKILLS,'audit')` returns 0 rows with `dropped >= 1`. Query A now leads `tech-debt` (3); `tear-sheet` is gone from query B | v1.34.0 | the floor or the empty branch being removed | verified |
| IS-2 | Non-discriminating words are measured against the roster, not listed | a term in >10% of descriptions is dropped and named in the output; a 61-skill fixture confirms `skill` is dropped and `xylophones` kept | v1.34.0 | a hand-kept stoplist being added beside it | verified |
| IS-3 | The ratio is off below 50 skills, because at six one match is 16.7% | a fixture asserts `weak == []` on the 6-skill corpus; without the floor the shortlist emptied itself — found by the fixture, not by reading | v1.34.0 | the corpus floor being removed | verified |
...
| RP-1 | A Russian verbal prefix no longer hides an advertised stem | `отрефактори модуль оплаты` and `зарефакторь этот класс` → `task-pipeline`, where both were `[]`; the bare `рефактор модуля` still matches itself | v1.39.0 | the boundary being restored before the stem | verified |
| RP-2 | Tolerance is bounded by a five-letter floor, and the bound is what makes it safe | `фикс` is four letters, so `зафиксируй результат` and `записал в блокнот` stay `[]` — recording a result is a different act from fixing a defect | v1.39.0 | the floor being lowered | verified |
| RP-3 | The prefix list admits no single letter and is ordered longest-first | both asserted against the shipped list and watched refusing a planted `с` and a shortest-first order; `о` was measured to earn no route and dropped | v1.39.0 | a one-letter prefix being added | verified |
| RP-4 | Widening the matcher spent no precision | route coverage 84 → 88 of 105 with *spoke where silence was right* unchanged at **0** | v1.39.0 | a silence probe starting to speak | verified |
...
## 2026-09-06/07 — v1.46.1, the audit wave

| id | Claim | Evidence | Shipped in | Invalidated by | Observed at |
|---|---|---|---|---|---|
| AW-1 | A system-generated turn neither routes nor spends the session opt-out | `triggers_test.js` asserts the marker-stripped control text ROUTES (so the silence is decided, not lucky), the marked text reaches `[]`, `render` prints nothing, and a notification quoting «без пайплайна» does not trip `optedOut`; `hooks_e2e_test.js` runs the real hook as a process on both payloads | v1.46.1 | a harness reframing its notifications without these markers | verified |
| AW-2 | «аудит скилов» reaches make-skill, and neither half shipped alone, with the live miss now in the corpus | `lib/triggers.js` gained `skill audit`/`аудит скилов` only after make-skill 0.27.1's description advertised them; the soundness fixture was WATCHED red on the 0.27.0 pin (`"skill audit" not in make-skill's description`, 1 failure of 49) and green after the re-pin. Two probes added to `route_coverage.js` — «проведи аудит скилов» and `skill audit of this repo` — and watched failing without the triggers: `match()` returns `["task-pipeline"]` with them removed and `["task-pipeline","make-skill"]` with them present; coverage 99/114 → 101/116, misses unchanged at 15 | v1.46.1 + make-skill 0.27.1 | either repository moving without the other | verified |
| AW-3 | The toolkit shortlist survives inflection in both alphabets | `toolkit_test.js`: «проведи аудит скилов» finds the skill advertising «скилла»; `termHit('a skill description','skills')` true; `termHit('дом и сад','домов')` false — the four-letter floor holds | v1.46.1 | the spread filter being bypassed for stemmed hits | verified |
| AW-4 | plugin.json and the marketplace entry cannot drift silently again | `check_shipped_descriptions_agree_across_manifests` was watched failing on the LIVE drift it was written for (task-pipeline 1.85.0, byte 298) before the member fixed it; three deliberate splits are declared with reasons, and a declared split with identical texts is refused too | v1.46.1 | the exceptions dict growing without reasons | verified |
| AW-5 | The hygiene guard passes the CLI's read-only mode and nothing wider | `hygiene_test.js`: `skills add ssheleg/super-ux --list` → null, `--list-agents` still refused — the exact token, not a prefix | v1.46.1 | flags being dropped before the decision again | verified |
| AW-6 | A non-address source is the declaration's defect, not the network's | `packs_test.js`: `checkReport` prints `UNDECLARED`, counts it as `bad` (editable here) and not `unreachable`; figma's source is owner-qualified and `pack --check` resolves it | v1.46.1 | the error-text contract with the fetch layer changing | verified |
| AW-7 | The router registry lost a third of its weight without routing worse | `test/evals/RESULTS.md` 2026-09-06: 13 probes × 2 arms, contested cells at 3 runs — recall equal-or-better everywhere (p03 3/3 vs 2/3, p09 exact-name), silence −1/3 on p02, p01 leaking identically ×3 in both arms; recall was measured on the 8,464 B eval arm (−43% vs 14,876); what ships is that arm PLUS the boundary headings and place sentences `router_texts_test.js` mandates — registry texts 14,853 → 11,878 B (−20%), whole managed block 23,494 → 18,570 B (−21%). All four mandated components hold (80 checks); outside the sentinels the operator file is byte-identical, 30 of the parser's 31 refusal phrases render (the absent «как есть» is a pre-existing never-advertised alias), and `routers --update` hashes identically on runs 1–3 | v1.46.1 | reintroducing per-router codas, or trimming past the boundary headings | verified |
| AW-8 | Eight members re-pinned at registry-confirmed releases | `npm view`: `@ssheleg/sheleg-dev` 0.11.8, `@ssheleg/make-skill` 0.27.1, `super-ux` 0.55.1, `agent-stack` 0.23.2, `@ssheleg/seo-aeo-audit` 0.25.11, `@ssheleg/agent-sync` 1.19.3, `task-pipeline-skill` 1.85.2, `sheleg-design-skill` 1.59.4; telegram-dev audited clean, unmoved | v1.46.1 | a pin moved without its registry read | verified |
| AW-9 | The machine-side findings were acted on, not filed | shadow check → 0 hits (3 symlinks removed); `agent_sync.py status` → `expired locks: none` (16 reaped by named operator decision); `/Users/sshlg/CLAUDE.md` counts re-measured and dated; claude-mem observer switched to openrouter (backup beside settings.json); the 340-skill triage is B-143, read-only until the operator marks | this session | the next hub install without a triage | verified |

Two tags were burned learning one lesson — sheleg-dev v0.11.6 and sheleg-design
v1.59.3, both tagged before their squash-merge landed — and both members carry
the burn in their own CHANGELOGs. The lesson is stamped in the retro: a tag is
cut from the verified merge commit whose subject names the PR.
