...
## 2026-09-06 — v1.46.0, the audit wave

| id | Claim | Evidence | Shipped in | Invalidated by | Observed at |
|---|---|---|---|---|---|
| AW-1 | A system-generated turn neither routes nor spends the session opt-out | `triggers_test.js` asserts the marker-stripped control text ROUTES (so the silence is decided, not lucky), the marked text reaches `[]`, `render` prints nothing, and a notification quoting «без пайплайна» does not trip `optedOut`; `hooks_e2e_test.js` runs the real hook as a process on both payloads | v1.46.0 | a harness reframing its notifications without these markers | verified |
| AW-2 | «аудит скилов» reaches make-skill, and neither half shipped alone, with the live miss now in the corpus | `lib/triggers.js` gained `skill audit`/`аудит скилов` only after make-skill 0.27.1's description advertised them; the soundness fixture was WATCHED red on the 0.27.0 pin (`"skill audit" not in make-skill's description`, 1 failure of 49) and green after the re-pin. Two probes added to `route_coverage.js` — «проведи аудит скилов» and `skill audit of this repo` — and watched failing without the triggers: `match()` returns `["task-pipeline"]` with them removed and `["task-pipeline","make-skill"]` with them present; coverage 99/114 → 101/116, misses unchanged at 15 | v1.46.0 + make-skill 0.27.1 | either repository moving without the other | verified |
| AW-3 | The toolkit shortlist survives inflection in both alphabets | `toolkit_test.js`: «проведи аудит скилов» finds the skill advertising «скилла»; `termHit('a skill description','skills')` true; `termHit('дом и сад','домов')` false — the four-letter floor holds | v1.46.0 | the spread filter being bypassed for stemmed hits | verified |
| AW-4 | plugin.json and the marketplace entry cannot drift silently again | `check_shipped_descriptions_agree_across_manifests` was watched failing on the LIVE drift it was written for (task-pipeline 1.85.0, byte 298) before the member fixed it; three deliberate splits are declared with reasons, and a declared split with identical texts is refused too | v1.46.0 | the exceptions dict growing without reasons | verified |
| AW-5 | The hygiene guard passes the CLI's read-only mode and nothing wider | `hygiene_test.js`: `skills add ssheleg/super-ux --list` → null, `--list-agents` still refused — the exact token, not a prefix | v1.46.0 | flags being dropped before the decision again | verified |
| AW-6 | A non-address source is the declaration's defect, not the network's | `packs_test.js`: `checkReport` prints `UNDECLARED`, counts it as `bad` (editable here) and not `unreachable`; figma's source is owner-qualified and `pack --check` resolves it | v1.46.0 | the error-text contract with the fetch layer changing | verified |
| AW-7 | The router registry lost a third of its weight without routing worse | `test/evals/RESULTS.md` 2026-09-06: 13 probes × 2 arms, contested cells at 3 runs — recall equal-or-better everywhere (p03 3/3 vs 2/3, p09 exact-name), silence −1/3 on p02, p01 leaking identically ×3 in both arms; recall was measured on the 8,464 B eval arm (−43% vs 14,876); what ships is that arm PLUS the boundary headings and place sentences `router_texts_test.js` mandates — registry texts 14,853 → 11,878 B (−20%), whole managed block 23,494 → 18,570 B (−21%). All four mandated components hold (80 checks); outside the sentinels the operator file is byte-identical, 30 of the parser's 31 refusal phrases render (the absent «как есть» is a pre-existing never-advertised alias), and `routers --update` hashes identically on runs 1–3 | v1.46.0 | reintroducing per-router codas, or trimming past the boundary headings | verified |
| AW-8 | Eight members re-pinned at registry-confirmed releases | `npm view`: `@ssheleg/sheleg-dev` 0.11.8, `@ssheleg/make-skill` 0.27.1, `super-ux` 0.55.1, `agent-stack` 0.23.2, `@ssheleg/seo-aeo-audit` 0.25.11, `@ssheleg/agent-sync` 1.19.3, `task-pipeline-skill` 1.85.2, `sheleg-design-skill` 1.59.4; telegram-dev audited clean, unmoved | v1.46.0 | a pin moved without its registry read | verified |
