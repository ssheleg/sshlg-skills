...
- **2026-09-06 — the constraint was one layer over from where the split was looking** (base `77fccd9`, umbrella v1.43.0 → v1.44.0). `B-141` priced four member releases on *free* description room. Free room is not spendable room: this repository ratchets the count of skills under **60** free characters and a ratchet may only fall, so a skill at 71 free can spend **11**. Measured by padding `task-pipeline`'s description 11, 12 and 30 characters in turn rather than by reading the guard. With spendable room computed the split reads **15 of 15 blocked** where it had read four rows as available work — and the row is closed by measurement rather than by doing the work it named. **The check that stops it recurring is a cross-file one**, because the threshold now lives in two files and nothing else compared them. **Divergence recorded, one:** that check's first draft looked for the substring `spendable`, which the file's own COMMENT satisfies, so the plant that renamed the function passed. Tightened to the declaration. **A guard that can be satisfied by prose about the code is a guard on the prose** — the third time this run that a check had to be re-scoped after its plant failed to land. Ledger edits under lease `SPENDABLE`; nothing retired — no cold trigger hit.

- **2026-09-06 — three members, and the pin refused the half I had shipped** (base `4407941`, umbrella v1.42.0 → v1.43.0, super-ux 0.53.0 → 0.55.0, make-skill 0.26.0 → 0.27.0, task-pipeline 1.84.1 → 1.85.0). **The completeness fixture shipped one release earlier earned its place on its first pin:** the moment `super-ux`'s submodule moved, it refused the tree because the member's description now advertised two phrases `lib/triggers.js` could not reach, and the soundness fixture then refused the stale phrase the description had dropped. A description edit and a trigger-table entry are one change living in two repositories, and neither half can ship alone now. **Divergence recorded, two, both mine and both caught one layer up.** (1) I released `super-ux` 0.54.0 with its ledger naming v0.53.0; this repository's cross-member ratchet caught it — *3 member ledgers describe a version older than they ship, and the ratchet stands at 2* — and the member's own gate had never asked. 0.55.0 repairs it and adds the check. (2) That check's first draft **crashed instead of refusing**, calling a `fail()` belonging to a sibling repository's validator; `rc=1` either way is precisely why the plant looked like it had worked, and a guard that crashes stops the suite before the rest of it can speak. **The through-line of the whole run: the backstop one layer up is right and is not a first line** — every one of these was caught above the repository that shipped it, which is exactly what `B-108` and `B-109` say and why they remain open. Ledger edits under lease `PIN-THREE`; nothing retired — no cold trigger hit.

- **2026-09-05 — the row described a class and the probe found an accident** (base `7cb1b76`, umbrella v1.41.0 → v1.42.0). `B-136` said a heredoc body fed to a non-shell is read as executable. Probed before writing anything: the `python3` body fired and the **identical `node` body did not**, because the python one happened to carry a word this guard reads as a write verb next to the path. **The row was right about the defect and wrong about its shape**, and the difference is only visible by running the guard rather than reading it. Fixed by importing `executablePart` from `lib/hygiene.js` — a second heredoc parser is what the copied-mechanism guard exists to catch. **No divergence in the change.** One thing worth keeping about the fixtures: the first copy-plant changed behaviour, so it failed the BEHAVIOUR fixture and told me nothing about the import one. Replaced with a behaviour-identical copy that delegates to hygiene and returns the same answer — it fails the import fixture alone. **A plant that fails two fixtures has proved neither.** Ledger edits under lease `HEREDOC-136`; nothing retired — no cold trigger hit.

- **2026-09-05 — two rows were waiting on a decision, and building was not the work** (base `b3d905c`, umbrella v1.40.0 → v1.41.0). `B-86` and `B-140` had each been open for weeks with the measurement already done: three connected surfaces named by no pack, and an accessibility lane no router owns. **Neither needed code; both needed somebody to choose and say so.** The family delegates accessibility permanently — the eleventh router was measured to be the wrong answer, not merely declined — and all three surfaces are refused with their own reasons. What is built is the CONTRACT around the decision: a delegation with no standing statement is refused, a lane both delegated and owned is refused, a surface verdict under 80 characters of reason is refused as *the silence it replaced*. **Divergence recorded, one.** My first two plants were invented packs and were refused by an earlier rule — an unowned lane's fallback must be an entry the pack declares — so they proved nothing about the rule under test; rebuilt by mutating the real pack, with an assertion that the unmutated pack passes. **A plant that trips the wrong rule is a green fixture guarding nothing**, which is `#91`'s shape on a different axis. Ledger edits under lease `DECIDE-86-140`; nothing retired — no cold trigger hit.

- **2026-09-05 — the question nobody asked was the converse of one that always passed** (base `770a87c`, umbrella v1.39.0 → v1.40.0). `triggers_test.js` had asked since it was written whether every trigger is a word the skill advertises, and it had never failed. **Nothing asked whether every advertised phrase is a trigger**, and 26 of 169 were not: `copywriting` promised «заголовок» and «кнопка», `agent-sync` promised «сабмодуль запушен», and the selector could name none of them. **A check that always passes is worth suspecting for that reason alone** — not because it is wrong, but because it may be asking the easy half. Twenty-four closed at zero description budget; the phrases were already paid for and only the table was missing them. **Divergence recorded, two, both mine and both caught by existing guards.** (1) I invented «отмена подписки» as the Russian for `cancel subscription` and the soundness fixture refused it before the commit — the guard I was extending caught the extension. (2) My first insertion matched `],` inside a comment instead of the array's closing bracket and left the module unparseable; found by requiring it, not by reading it. Two phrases are excused with reasons asserted rather than counted, because an excuse list with no reasons passes by excusing everything. Ledger edits under lease `ADVERTISED-GAP`; nothing retired — no cold trigger hit.

- **2026-09-06 — a rule would not have helped, and the proof is that the rule existed** (base `a4e52d1`, umbrella v1.44.0 → v1.45.0). The operator asked whether a *rule in a skill* would stop credentials leaking. It would not: the prohibition was already in their global instructions and an agent holding it leaked the same live key twice in one session, because `${VAR:-fallback}` expands to the value and therefore disguises a leak as a presence check. The refusal went where the pack already refuses — `PreToolUse` — with five rules and a `PostToolUse` scanner for what the first layer cannot foresee. **Divergence recorded, three, and every one was my own instrument.** (1) The first guard denied three of the four remedies its own message prints, because it asked whether a secret-ish expansion appeared anywhere in the line rather than whether it was an argument to a printing command; the ALLOWED half of the suite caught it, which is why that half exists. (2) Splitting the command on `|` to find segments destroyed the pipe that makes `env | grep -c` safe — the rule now tests the original string. (3) The suite reported in prose instead of `OK (N checks)`, so 33 fixtures were invisible to the ratchet; correcting the format showed the count **5 short**, which surfaced a cherry-pick stopped at a conflict whose second commit had never applied. **The tree looked finished and was not, and only a number nobody typed by hand disagreed.** Also owned: the previous session committed to this repository with coordination on and **took no lease**, which is why `keep/other-session-secrets` had to exist at all; this work was done under `SECRETS-GUARD`.

- **2026-09-05 — two rows said settled, and neither was** (base `1702bfa`, umbrella v1.38.2 → v1.39.0). `B-84` recorded the matcher defects as settled in v0.85.1, and a fourth class was untouched: `phrasePattern` required a word boundary before the stem, so every Russian prefixed form of an advertised verb reached nothing while the bare stem routed. The word *settled* is what kept anyone from looking. `B-141` priced its work at four member releases on four figures, and **two did not survive re-derivation** — `super-ux` was given 341 free characters against a fronting skill with **5**, `sheleg-dev` 149 against **7** — both wrong in the direction that made the work look doable. **Divergence recorded, three, and every one was my own instrument.** (1) A naive `split("|")` over the board reported two closed rows as open, which is exactly the defect `B-94` fixed in both board guards. (2) The first cross-check used `$` under `/m` — end of LINE — and measured a multi-line description as its first line, reading 72 characters for `stripe-billing` instead of 963; that is `B-63`'s defect, and I committed it inside a comment warning against it. (3) The order fixture's assertion was inverted and accused a correct list. **Each was caught by crossing to the instrument that decides** — the board guard's own splitter, `audit_skill.py --house`, the fixture's own plant. **Also found and not swept:** two commits by another session sat unpushed on local `main` (`b7d3993`, `ca3daed` — a credential redactor with its own tests). They were not absorbed into this release: a named branch `keep/other-session-secrets` holds them and this work was rebased onto the released tag instead. Ledger edits under lease `ROUTE-PREFIX`; nothing retired — no cold trigger hit.

- **2026-09-05 — the tag was proved against its own tree, which is the rule two burned tags bought this morning** (base `e6b0cff`, umbrella v1.38.1 → v1.38.2, task-pipeline 1.84.0 → 1.84.1). The member's README stated its conformance to the Agent Skills standard in four hand-written figures, and one was false — the references were claimed reachable *within two hops* when all 38 are linked **directly** from `SKILL.md`. The fix computes all four from the tree and **asserts all four were parsed** before comparing, because a parser that silently matched three would gate on three and print green. **No divergence in the change itself; the divergence was in how it shipped.** Two tags were burned earlier in the day on a stamp carrying an em dash where a commit sha belongs, and the remedy — *cut the tag locally, run `test:all` against its tree, push nothing* — was already written in that repository from five earlier burns. This time it was followed before the push rather than after: rc 0, 424 guards provably rejecting their planted defect, then the tag left the machine. **Also corrected here: `npm view task-pipeline` is a different package.** The family's is `task-pipeline-skill`, and the wrong one answered `0.1.0` without erroring — reading the registry only helps when it is the registry entry of the subject. Ledger edits under lease `PIN-1841`; nothing retired — no cold trigger hit.

- **2026-09-05 — the measure was an upper bound, and a board row rested on it for four days** (base `c293928`, umbrella v1.38.0 → v1.38.1). Loop task 4, and the premise check paid for the fourth time. `B-138` asked to extend the copied-mechanism guard to `.js` and `.yml`, priced at **seven member releases**, on `quick_ratio` figures of 0.918–0.977 for `installer_test.js`. `quick_ratio` compares character multisets and IGNORES ORDER — it is an upper bound. Exactly: **0.182–0.815, and not one of 21 pairs reaches the 0.90 floor.** The extension would have demanded seam declarations for files sharing eighteen percent of their text. **And the guard decided on the bound too**, gating five of its six subjects above a floor they do not meet — `residue.py` at 0.943 with a true 0.665, four copies told to declare a seam nobody could write truthfully. Fixed three ways: the bound is a prefilter and the exact ratio is the verdict; **all pairs** are compared rather than base-against-the-rest, because the base was the alphabetically first REPOSITORY and `social_preview.py` measures 0.913 or 0.828 depending on which member sorts first; and the guard asserts it holds at least one subject, since deciding correctly dropped it from six to one — `#91` applied to this guard on the day its rule shipped. **No divergence in the build. The divergence is the pattern: four audit findings, four premises re-derived, and three of the four moved.** An audit that stops at the first confirming measurement produces exactly this, and the only thing that caught it every time was asking the subject with the instrument that decides rather than the one that is cheap. Ledger edits with no lease; nothing retired.

- **2026-09-05 — the reserve everyone spent, and the premise that was two-thirds wrong** (base `825f09e`, umbrella v1.37.0 → v1.38.0). Loop task 3. **Premise re-derived before designing, because two of two audit findings had already been my measurement errors** — and this one held where it counted and broke where it mattered. Held: 12 of 28 skills within 60 characters of the 970 reserve, `agent-interop` at exactly 970, reproduced independently against `audit_skill.py`'s own figures. Broke: *routing cannot be widened* is **false in general**. Of `route_coverage.js`'s 18 remaining misses, **8 are blocked by the budget and 10 are not blocked at all** — `super-ux` has 341 characters free and `copywriting` 336, and their triggers were simply never added. **A premise that is true for a third and stated for the whole is the shape an audit produces when it stops at the first confirming measurement.** So the umbrella ratchets the blocked half and `B-141` carries the ten with each route's free characters. **No divergence in the build; the divergence was in the premise, and re-deriving it is what the previous two iterations bought.** The check ratchets rather than fails, recorded with its reason: the descriptions belong to each member and are trimmed on its own release, and an umbrella turning twelve skills red would be failing for work it does not own. Ledger edits with no lease — the loop holds none; nothing retired.

- **2026-09-05 — the audit's first finding was a fact about my laptop** (base `96af3af`, umbrella v1.36.0 → v1.37.0). Task 1 of the loop: yesterday's family audit reported three `.pyc` shipping in two members. **The published tarballs carry zero** — `npm pack --dry-run` lists `__pycache__` on a machine where the tests have run, and `npm publish` runs from a fresh CI checkout where nothing has executed Python. **Standing instruction #10, committed while auditing**, which is the one place it costs most: an audit's only value is measurement, and this one measured the wrong artefact. What survived is better than what was claimed — the cleanliness is INCIDENTAL, two members already carry the exclusion, and the seam was written nowhere, which is `B-138`'s shape rather than a packaging bug. The check reads the CONFIG, which every clone has. **Divergence recorded, one, and the downward plant is what found it:** the first matcher tested only the `.pyc` path against the negations, and `fnmatch` has no `**`, so a pattern ending in `__pycache__` cannot match a path ending in `x.pyc` — it passed `agent-sync` by accident of that member's SECOND negation entry, and adding the pair to `make-skill` should have dropped the count and did not. **A one-way ratchet has a blind side and this is what it looks like.** Both plants now assert they landed before the verdict is read, which is `#94` applied to my own work on the run that shipped `#94`'s rule. Ledger edits under lease `ISSUE-101` released earlier; nothing retired — no cold trigger hit.

- **2026-09-05 — the last of the twenty-eight, and it was the one I argued against** (base `f979d7c`, umbrella v1.35.0 → v1.36.0). `#101` asks how a retrospective tells a router **obeyed from the repository** from one skipped for budget: both look like a router that owned the ground and was not invoked. I had recommended closing it twice — a `.claude/routers.json` contract every project must understand is a mechanism introduced on the second example, not the first — and the operator said do it. **Building it changed my reading of it**, which is the part worth recording: the weight I objected to was in the DECLARATION, and the declaration is the half that does not matter. Resolving it is the value — every path opened, every `#marker` looked for, so a stylesheet that survived a refactor which removed everything the router put in it reports FALSE rather than satisfied. **A document asserting a route was honoured when it was not is worse than the silence it replaced**, which is the same sentence as every one of the twenty-seven before it. The issue's own smaller alternative ships beside it, because it works where no JSON exists: the block now requires a router obeyed from the tree to name the files it was read from. **No divergence.** Two refusals are stated in the command's own output rather than only in its source — the guards are not run, because executing code out of somebody's repository is a different act with a different risk; and a satisfied row means the doctrine is READABLE here, not obeyed. Ledger edits under lease `ISSUE-101`; nothing retired — no cold trigger hit.

- **2026-09-05 — twenty-seven of twenty-eight, and the pass kept demonstrating the issues it was closing** (base `2ec4f6e`, umbrella v1.34.1 → v1.35.0, task-pipeline 1.82.4 → 1.84.0, super-ux 0.52.5 → 0.53.0). The 28 filed issues were one subject in many titles — what a check is allowed to conclude — so the fifteen `task-pipeline` ones shipped as a single release: `learned.md` 22 → 32 with two existing rules strengthened rather than duplicated, three sub-canons under the canons they refine, and the fixer's half of the effort rule. **Divergence recorded, three, and each was an issue in the set being closed.** (1) **`#80` was broken on the run that closed its neighbours**: a run stamp carried `—` where its commit belongs, the release gate refused the tag's own tree, and the tag could not be repaired because a repository rule forbids deleting one — correctly, a tag being a promise about a tree. **Two tags burned.** The remedy was already written in that repository's `## Releases that carry no stamp` from five earlier burns — *cut the tag locally, run `test:all` against its tree, push nothing* — and I read it only after repeating it twice. (2) **`Closes A, B, C` closes only A**, a class nobody had filed: four issues were reported closed to the operator and three were open, invisible from the PR, found by recounting. (3) **Rule 8 caught me inside the file that states it** — `Numbers issued so far: 22` over a table reaching 32. **The through-line worth keeping: every one of these was caught by asking the SUBJECT rather than the report** — recount the board, read the registry not the pipeline, print the value before depending on it, run the tag's suite against the tag's tree. Ledger edits under leases `ISSUE-86`, `AUDIT-BATCH`, `PIN-1840`; nothing retired — no cold trigger hit.

- **2026-09-04 — two more of the twenty-eight, and a rule about reports that had no exception** (base `3679232`, umbrella v1.34.0 → v1.34.1, agent-sync 1.19.1 → 1.19.2). `#104` closed in the member: an expired foreign lease printed at every SessionStart for a fortnight, sixteen of them, none of them clearable by the operator who kept reading them. `#103` closed as **keep it** — the issue asks that a column not be tidied away, every artefact it names was resolved at HEAD, and the record is the receipts rather than an agreement. **Also changed outside any repository: the operator's own standing rule on visual reports.** It read *"Всегда, для всех проектов … пишу локальный HTML"* with no exception, so a report was the default output of any visual-shaped result; it now requires an explicit ask, lists what counts as one, and says to answer in the conversation and ask in one line when unsure. Lines 1–554 of that file are byte-identical, checked with `diff` rather than asserted. **Divergence recorded, one, and it is instruction #6 from the far side:** the new ledger section did not quote `PASS: agent-sync v1.19.2`, so the self-test plant that rewrites that exact line in the newest section had nothing to rewrite and reported ITSELF undetected — `SELF-TEST FAILED — undetected: ['the ledger names a version that did not ship']`, on a validator that was fine. A plant anchored on a string the author is free not to write is a plant that reports the guard as broken; the section quotes the validator's own version line now. Ledger edits under leases `ISSUES-UMBRELLA` and `PIN-AS-1192`; nothing retired — no cold trigger hit.

- **2026-09-04 — four filed issues, each one already having cost something** (base `855d9f9`, umbrella v1.33.1 → v1.34.0). The family's agents filed 28 issues overnight; four belong here and every one of them named a session where the defect had already been paid for — a shortlist that put Zoom SDK skills above a backlog tool on the words *after* and *full*, a pack that printed `a11y GAP` and was read by a run that then shipped two public pages without opening the lane, 173 files of somebody's local skill install swept into a product commit about an MCP server, and a router table sentence that excluded the case of building the thing an agent connects to. **Divergence recorded, two, and both were the fixtures refusing my first answer.** (1) The non-discriminating-word filter is a ratio, and I applied it unconditionally: over the six-skill test corpus a single match is 16.7%, so every term cleared the threshold and the shortlist emptied itself — five existing fixtures went red at once, which is the uniform-failure shape instruction #11 names, and the instrument was mine. The ratio is off below fifty skills now, stated as a rule about denominators rather than as a magic number. (2) The new lane rule refused the fixtures' OWN demo pack, correctly: several cases replace `entries` wholesale and the demo's hardcoded fallback stopped resolving. Deriving the fallback from the pack's actual entries fixed it, and the principle is worth keeping — **a fixture whose scaffolding fails the rule under test tells you nothing about the rule.** Ledger edits under lease `ISSUES-UMBRELLA`; nothing retired — no cold trigger hit.

- **2026-09-03 — I annotated a board row and deleted its state** (base `855d9f9`, umbrella v1.33.0 → v1.33.1). Asked what came next, the first act was to recount the board — and it read **8 open where it had read 9 an hour before**. The missing row was `B-84`, whose status cell I had annotated in the previous change: accurate content, prepended, pushing `open` behind three hundred characters. **A status cell states its verdict first or the row has none**, because every reader of this board anchors on it. Neither existing guard could see the defect — one looks for a row claiming TWO verdicts and this one claimed none, the other anchors `^waived` and did not match — so both ask what the cell says and neither asks where it says it, which is `B-108`'s shape exactly. **No divergence in the fix; the divergence was the edit that caused it, one change earlier**, and the lesson is narrow enough to keep: an annotation is an append, never a prepend, wherever position carries meaning. The guard was watched firing on the planted prepend and silent on the repair, and the count is 9 again. Ledger edits under lease `B84-CELL`; nothing retired — no cold trigger hit.

- **2026-09-03 — I wrote a paragraph about prompts that could not arrive** (base `2ec4f6e`, umbrella v1.32.2 → v1.33.0, sheleg-design 1.59.1 → 1.59.2). Asked to check the work and fix what was unfinished, the first probe was the one closest to what had been requested: do the words the new router paragraph names actually route? **`редизайн сайта` → nothing. `свёрстай дашборд` → nothing. `переделай дизайн` → nothing.** The block says *before design, redesign, layout, front-end or mobile-UI work* and the trigger layer fires only on words a skill claims about ITSELF, so the paragraph was inert for its own subject and had shipped that way in v1.32.0. **The lesson is narrow and worth keeping: writing a route's INSTRUCTIONS and writing its ADDRESS are two changes, and doing the first alone produces something that reviews perfectly.** Fixed in the member (four triggers, bought by trimming prose) and here. **The wider half came out of the same probe: `ё` was hiding two routers.** Russian is normally typed without it, so `звезды телеграм` and `отчет о приемке` — the ordinary spellings of two shipped triggers — reached nothing while the ё forms routed. `normalise()` and `phrasePattern` fold now, which cost no description budget and fixed a class. **Divergence recorded, one, and the existing suite caught it:** folding only the pattern made `светлая/тёмная тема` stop matching ITSELF, because `matches()` is the lower-level door and its callers bring their own text — `trigger … no longer matches itself`, `1 failure(s) out of 43 checks`, from a fixture written long before this change. It folds its own text now, and a second fixture asserts no two triggers collide once folded, because the fold does spend precision on pairs like `всё`/`все`. Coverage 74 → **81 of 99**, with three new probes deliberately left MISSING and the reason written beside them. Ledger edits under lease `ROUTE-GAP`; nothing retired — no cold trigger hit.

- **2026-09-03 — the pack could rot and nothing here would notice** (base `96af3af`, umbrella v1.32.1 → v1.32.2). The ladder walk's third finding, and the sharpest of the three because it was a defect in the thing just shipped: `lib/packs.js` names ten outside repositories, `--check` resolves them, and **nothing ran `--check`**. `docs/DOCMAP.md` said to run it when an entry is ADDED — but an address rots without anyone touching the entry, which is precisely what the pack was built to expose in two published articles where three of five addresses had already moved. **The pack was one release old and already carried the failure mode it exists to catch in other people.** Closed with CI rather than a board row, in the step beside `check_pins.py` and borrowing its exit split wholesale: **2, never 1**, because an upstream rename is not this commit's defect and a gate that fails a build for somebody else's release gets re-run rather than read — a lesson this repository had already paid for once (`B-13`). `unreachable` never counts, so an offline machine or a spent API quota cannot turn a network outage red. **The cost is stated in the CHANGELOG rather than implied: nothing BLOCKS on a rotted address**, and the choice was between a warning that is read and a failure that is bypassed. `checkExit` was watched firing against a LIVE rename — `accesslint/claude-marketplace` still 301s — so exit 2 came from a real redirect and not a synthetic fixture, and a fixture refuses 1 across six input shapes including the empty list. **No divergence:** the finding came from the walk exactly as stage 10 describes, and the fix followed an existing pattern in this repository instead of inventing one. Ledger edits under lease `PACK-ROT`; nothing retired — no cold trigger hit.
