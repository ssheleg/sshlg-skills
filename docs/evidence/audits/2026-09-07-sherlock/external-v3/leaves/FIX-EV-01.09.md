# FIX-EV-01.09 — Outcome corpus: evidence-docs
...
### Create `repo://task-pipeline/evals/cases/evidence-docs.json`
...
Тестовый artifact: `repo://task-pipeline/test/audit_regressions/fix-ev-01.09.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.
...
2. Для evidence-docs записать минимум positive, negative и ambiguous/no-op case из собственных audit findings; отдельно with/without skill. Не менять production integration для удобства grader.
...
Case manifest включает actual output oracle и raw result; evidence-docs не проходит лишь по названию. Недоступный live host = NOT_RUN.
...
Appendix и полный parent contract: [leaf JSON](FIX-EV-01.09.json), [parent](../parents/FIX-EV-01.json). Полный audit не required prompt input.
