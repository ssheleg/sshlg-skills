...
```text
722:   const p = String(prompt || '');
723:   return SYSTEM_TURN_MARKERS.some((m) => p.includes(m));
724: }
725:
726: function optedOut(prompt) {
727:   // A refusal is the operator's word. A system turn carries nobody's word, so
728:   // it can neither refuse a route nor spend the session's sticky opt-out.
729:   if (isSystemTurn(prompt)) return false;
730:   const p = normalise(prompt);
731:   return REFUSALS.some((r) => refuses(p, r));
732: }
733:
734: /**
735:  * The routes a prompt asks for, in registry order.
736:  *
737:  * Empty on a question, on an opt-out, and on a prompt with no trigger at all —
738:  * three different reasons that must all produce silence rather than a guess.
739:  */
740: function match(prompt) {
741:   if (!prompt || isSystemTurn(prompt) || optedOut(prompt)) return [];
```
...
Тестовый artifact: `repo://sshlg-skills/test/audit_regressions/fix-rt-02.02.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.

## Шаги

1. Прочитать primary packet и проверить hash актуальных edit targets; upstream outputs materialize до claim.

2. Сессионный отказ сохранять до отзыва; общий session waiver отдельное явное действие пользователя.
...
Следующий ход сохраняет scope; revoke восстанавливает только указанный маршрут.

Positive и исходный failure case проверяются по поведению, не по повторению слов инструкции.

## Входы и handoff

- `FIX-RT-02.01` (data): Consumes the preceding contract/state/API decision described in this outcome.
...
Appendix и полный parent contract: [leaf JSON](FIX-RT-02.02.json), [parent](../parents/FIX-RT-02.json). Полный audit не required prompt input.
