# FIX-DV-03.02 — Reconcile-first compensation
...
Тестовый artifact: `repo://sheleg-dev/test/audit_regressions/fix-dv-03.02.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.

## Шаги

1. Прочитать primary packet и проверить hash актуальных edit targets; upstream outputs materialize до claim.

2. По умолчанию восстанавливать БД из подтверждённого Stripe state; refund только отдельная явно заданная business policy.
...
Изменение local count не называется refund; repeated reconciliation не снимает деньги повторно.

Positive и исходный failure case проверяются по поведению, не по повторению слов инструкции.

## Входы и handoff

- `FIX-DV-03.01` (data): Consumes the preceding contract/state/API decision described in this outcome.
...
Appendix и полный parent contract: [leaf JSON](FIX-DV-03.02.json), [parent](../parents/FIX-DV-03.json). Полный audit не required prompt input.
