# FIX-DV-19.06 — Artifact eval: error-tracking
...
### Create `repo://sheleg-dev/evals/cases/fix-dv-19.06.json`
...
Тестовый artifact: `repo://sheleg-dev/test/audit_regressions/fix-dv-19.06.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.

## Конкретные cases этого skill

DV-15: Эталонный string scrubber пропускает распространённый Redis password и bot-token URLs — Canary matrix fake Redis/AMQP/Postgres URLs, encoded credentials, Telegram path, access-token query, nested events; ни один fake secret не доходит до test transport.
...
2. Добавить offline artifact/fault-injection corpus error-tracking, привязанный к соответствующим DV findings; protocol/provider stubs не являются oracle сами для себя.
...
Appendix и полный parent contract: [leaf JSON](FIX-DV-19.06.json), [parent](../parents/FIX-DV-19.json). Полный audit не required prompt input.
