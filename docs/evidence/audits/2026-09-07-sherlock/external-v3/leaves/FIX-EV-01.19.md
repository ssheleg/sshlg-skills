# FIX-EV-01.19 — Outcome corpus: frontend-performance
...
### Create `repo://sheleg-dev/evals/cases/frontend-performance.json`
...
Тестовый artifact: `repo://sheleg-dev/test/audit_regressions/fix-ev-01.19.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.
...
2. Для frontend-performance записать минимум positive, negative и ambiguous/no-op case из собственных audit findings; отдельно with/without skill. Не менять production integration для удобства grader.
...
Case manifest включает actual output oracle и raw result; frontend-performance не проходит лишь по названию. Недоступный live host = NOT_RUN.
...
Appendix и полный parent contract: [leaf JSON](FIX-EV-01.19.json), [parent](../parents/FIX-EV-01.json). Полный audit не required prompt input.
