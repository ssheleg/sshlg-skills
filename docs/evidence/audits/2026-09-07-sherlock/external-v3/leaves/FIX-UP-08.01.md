# FIX-UP-08.01 — HostContext root precedence
...
142:   };
143:
144:   // A copy is a shadow only where a plugin of the SAME MEMBER is installed —
145:   // which is not the same question as "is this run touching plugins". That
146:   // proxy is what let `update --no-claude` create a copy and walk away from
147:   // it, beside a live plugin, serving a frozen version forever.
148:   const members = SKILLS.map(s => ({
149:     name: s.name,
150:     marketplace: s.pluginInstall.split('@')[1],
151:     skillNames: s.skillNames,
152:   }));
153:   const pruned = [];
```

Тестовый artifact: `repo://sshlg-skills/test/audit_regressions/fix-up-08.01.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.

## Шаги

1. Прочитать primary packet и проверить hash актуальных edit targets; upstream outputs materialize до claim.

2. Explicit root > documented host env > platform default; host existence отдельная probe.
...
CODEX_HOME/CLAUDE_CONFIG_DIR меняют только выбранный root; spaces preserved.
...
Appendix и полный parent contract: [leaf JSON](FIX-UP-08.01.json), [parent](../parents/FIX-UP-08.json). Полный audit не required prompt input.
