# FIX-AS-01.02 — Reconciliation and serialization
...
Тестовый artifact: `repo://agent-stack/test/audit_regressions/fix-as-01.02.py`. Это planned Create/extend; wiring в существующий runner принадлежит этой же правке.

## Шаги

1. Прочитать primary packet и проверить hash актуальных edit targets; upstream outputs materialize до claim.

2. Unknown проверять по idempotency key; сериализовать tenant transfer state через revision/CAS; compensate только собственную подтверждённую операцию.
...
Concurrent/late HTTP responses не компенсируют чужой ledger; repeated reconciliation идемпотентен.

Positive и исходный failure case проверяются по поведению, не по повторению слов инструкции.

## Входы и handoff

- `FIX-AS-01.01` (data): Consumes the preceding contract/state/API decision described in this outcome.
...
Appendix и полный parent contract: [leaf JSON](FIX-AS-01.02.json), [parent](../parents/FIX-AS-01.json). Полный audit не required prompt input.
