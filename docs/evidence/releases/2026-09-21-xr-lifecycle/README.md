...
Concurrent integration: main advanced through [bf6a78d](https://github.com/ssheleg/sshlg-skills/commit/bf6a78d32de8fc2d5851e1313cba47db8a70d82e), making Skills primary, preserving a separate Harness section and marking Foundry in development. This branch merged that work without replacing it. The XR scenario became SCN-005 to retain the incoming Foundry SCN-004. The combined run passed **88 suites / 1031 fixtures**, including **45 site checks**. XR and home were reviewed at 320px without horizontal overflow; Skills/Harness navigation and Foundry's development state were observed.

Optional documentation audit is **not green**. Before the concurrent merge, brand_lint found 9 errors/2 warnings in the inherited pack format. The incoming change fixes that format and scans generated HTML, with its parser limitations documented in docs/brand/README.md. On the combined tree the actual report is 1095 errors/521 warnings: B030 973 (numeric claims, including CSS/HTML values), B062 121 (dash style), B021 1 (the inherited Update string), B022 520 and B003 1. These are not silently waived as all false positives: visible copy was reviewed, but parser correction and the broader pre-existing style/registry debt remain a separate task. ux_lint reports no errors/one inherited missing-web-surface warning. No global brand-lint pass is claimed.
