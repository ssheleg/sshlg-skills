 ...
 * memory was of the family — the packs whose routing block it had just read. On this
...
function classify(skills, familyIds) {
  ...
  // FIX-RT-03.01 — the roster is what is CALLABLE on the active host. An entry
  // discovered but not callable (a broken symlink, a same-named foreign target,
  // a dir with no live SKILL.md) is counted apart, never presented as
  // available. When `callable` is undefined (an older caller), treat it as
  // callable so behaviour is unchanged.
  const isCallable = (s) => s.callable !== false;
  const uncallable = [];

  for (const s of skills || []) {
    if (!isCallable(s)) { uncallable.push(s); continue; }
  ...
  return {
    ...
    foreign,
    total: mine.length + foreign.reduce((n, g) => n + g.count, 0),
    uncallable: uncallable.sort((a, b) => a.id.localeCompare(b.id)),
  };
}
...
/**
 * `ё` folds to `е` on both sides — the same fold `lib/triggers.js` bought on
 * 2026-09-03, for the same reason: Russian is normally typed without it.
 */
function fold(s) {
...
function terms(query) {
  return [...new Set(fold(query).match(/[\p{L}\p{N}][\p{L}\p{N}-]{2,}/gu) || [])]
    .filter((t) => !STOP.has(t));
}

/**
 * A query word arrives inflected; a description carries one form. An exact
 * substring misses «скилов» against a description advertising «скилл», and
 * `skills` against `skill` — measured 2026-09-06, when the family's own audit
 * query matched NOTHING out of 530 skills because every Russian noun in it was
 * declined. The fix is a progressive prefix, not a language table that would
 * rot: try the whole word, then strip up to three trailing letters with a floor
 * of four, and let `FLOOR` plus the spread filter eat what the loosened match
 * lets through — a shortlist ranks, it never decides.
 */
function termHit(hay, term) {
  if (hay.includes(term)) return true;
  if (term.length < 5) return false;
  const floor = Math.max(4, term.length - 3);
  for (let n = term.length - 1; n >= floor; n -= 1) {
    if (hay.includes(term.slice(0, n))) return true;
  }
  return false;
}
...
function discriminating(skills, words) {
  ...
  for (const w of words) {
    let hits = 0;
    for (const s of skills) {
      // The same loosened match `rank` scores with, or a word the stem makes
      // common would pass the spread filter it should have failed.
      if (termHit(fold(`${s.id} ${s.description || ''}`), w)) hits += 1;
    }
...
function rank(skills, query, limit, familyIds) {
  ...
  let dropped = 0;
  for (const s of skills || []) {
    const hay = fold(`${s.id} ${s.description || ''}`);
    const hits = words.filter((w) => termHit(hay, w));
...
function report(skills, familyIds, opts) {
  const o = opts || {};
  const { family, foreign, total, uncallable } = classify(skills, familyIds);
  const lines = [];

  const host = o.host || 'claude';
  lines.push(`Toolkit — ${total} skills CALLABLE on the active host (${host})`);
  lines.push('');
  ...
  if (uncallable && uncallable.length) {
    lines.push('');
    lines.push(`  discovered but NOT callable on ${host} (${uncallable.length})`);
    for (const s of uncallable) {
      lines.push(`    ${pad(s.id, 26)}${s.provenance || ''} — no live SKILL.md resolves`);
    }
    lines.push('  These are inventory, not availability: a broken symlink, a');
    lines.push('  same-named foreign target, or a directory with no SKILL.md. Other');
    lines.push('  hosts\' skills remain viewable as inventory, never as this host\'s.');
  }
...
module.exports = {
  classify, rank, terms, termHit, report, readSkills,
};
