...
function hostRoot(target, home, opts) {
  ...
  if (fromEnv) return fromEnv;                 // verbatim: spaces preserved
  return path.join(home, target.dir);
}
...
function apply(opts) {
  ...
  for (const t of hosts) {
    const dir = hostRoot(t, home, opts);
...
module.exports = { apply, applyOne, applyCursor, writeOptOut, protect, hostRoot, TARGETS, EMPTY_BLOCK };
