 ...
 */
function line(installed, latest, config) {
  if (compare(installed, latest) !== 'behind') return '';
  const head = `[sshlg-skills] A newer set is out: ${latest} (you have ${installed}). `;
  // Two lines used to say this — one telling the operator to run the command,
  // one saying it installs itself when they go idle. That is contradictory
  // advice, and the reader has to work out which half is true. When the
  // unattended path is on, ONE line says what will happen and how to stop it;
  // when it is off, one line says what to run. Never both.
  const c = (config && config.update) || {};
  if (c.auto === 'off') {
    return head + 'The nine packs move together — `npx sshlg-skills@latest update`.';
  }
  // "move together" is the load-bearing half and a fixture holds it: it is WHY
  // there is one command and no per-member argument, and a notice that drops it
  // reads as nine independent updates the operator could pick from.
  return head + 'The nine packs move together and install when you go idle. '
    + 'Sooner: `npx sshlg-skills@latest update` · never: `config set update.auto off`.';
}
...
function plan(installed, cache, now, intervalMs, config) {
  ...
  // ONE line about the next set, not two. The disclosure that the unattended
  // path exists is folded into it, because a standing announcement beside an
  // actionable notice is the half a reader skims.
  const behind = line(installed, c.latest, config);
...
function ranLine(cache) {
  ...
  // A run that succeeded and could not read the version it reached is still a run
  // that may have moved the set, and the restart is the part the operator acts on.
  // Saying nothing here is how FIX-RT-01 stayed invisible: the version was
  // unreadable in the wired runtime, so every outcome looked like "nothing moved".
  if (r.ok && !r.to) {
    return '[sshlg-skills] An unattended set update ran; the version it reached '
      + 'could not be read. Restart Claude Code to load whatever it installed — '
      + 'skills are read at session start.';
  }
  return '';
}
