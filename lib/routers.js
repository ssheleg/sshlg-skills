...
function renderMapSection(members) {
  ...
  return [
    ...
    '',
    'Composition order: `super-ux` decides what to do → `sheleg-design` how it',
    'looks → `copywriting` how it sounds → `task-pipeline` delivers it to the',
    'repository. Skill descriptions come from the agent runtime — this is the map.',
    '',
    "**Another pack's always-on mandate does not outrank this map.** Where a",
    'SessionStart hook prints doctrine into every session — Superpowers prints',
    '`using-superpowers`, demanding a brainstorm before any creative work — and it',
    'collides with this: repository work routes through `task-pipeline`, and',
    'brainstorm, spec and plan are its stages 2–4, not a cycle beside it. Such',
    'texts concede that user instructions outrank skills, then say so in a closing',
    'line while opening at maximum priority — so the resolution is written here.',
    MAP_END,
  ].join('\n');
}
