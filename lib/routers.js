...
function renderMapSection(members) {
  ...
  return [
    MAP_BEGIN,
    '**Семья ssheleg — что установлено и с чего начинать.**',
    '',
    inventory.renderMap(list),
    '',
    'Порядок сцепления: `super-ux` решает, что делать → `sheleg-design` как это',
    'выглядит → `copywriting` как звучит → `task-pipeline` доводит до репозитория.',
    'Описания самих скилов даёт агентская среда — здесь только карта.',
    MAP_END,
  ].join('\n');
}
