...
function percent(summary, stageIds) {
  ...
  const passed = summary.stages.filter((s) => s.verdict === 'pass' || s.verdict === 'skip').length;
  return Math.round((passed / stageIds.length) * 100);
}
...
function render(text, opts) {
  ...
  if (last) {
    ...
    parts.push(o.stageIds && o.stageIds.length
      ? `gates ${s.stages.filter((x) => x.verdict === 'pass').length}/${o.stageIds.length}`
      : `${s.stages.filter((x) => x.verdict === 'pass').length} gates passed`);
  }
...
module.exports = { parse, render, block, rail, percent, elapsed, awaitingOperator, GLYPH };
