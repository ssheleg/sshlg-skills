...
const ROUTES = {
  ...
  'sheleg-design': {
    ...
    triggers: [
      ...
      // check is for.
      //
      // `дизайн` is deliberately NOT here, and cannot be: it is a substring of this
      // route's own refusal «без дизайна», so the fixture rejects it — a trigger
      // inside a refusal makes the refusal unsayable. The measured consequence,
      // recorded rather than guessed: `сделай дизайн лендинга` matches NO route at
      // all, not `super-ux` as a first draft of this comment claimed. Whether that
      // phrase should open the whole chain is a composition question this row does
      // not decide; it is on the board.
    ],
