...
function classify(prompt) {
  if (!prompt || isSystemTurn(prompt) || optedOut(prompt)) {
    return { intent: 'none', clauses: [], effects: { write: false, report: false }, facets: [] };
  }
  const clauses = splitClauses(prompt).map((text) => ({
    text,
    intent: clauseIntent(text),
    routes: Object.keys(ROUTES).filter((name) =>
  ...
  return { intent, clauses, effects, facets };
}

module.exports = {
  match, render, sessionNote, isQuestion, optedOut, matches, isSystemTurn,
  stemRu, stemLat, phrasePattern, classify, splitClauses,
