 ...
 */
const turnstate = require('./turnstate.js');
...
const ROUTES = {
  ...
  'sheleg-design': {
    ...
    triggers: [
      ...
      // Advertised in this skill's own `Triggers -` list and absent from this
      // table: the description promised a phrase the selector could not name.
      'investor deck as a web page', 'веб-презентация',
    ],
  ...
  'make-skill': {
    ...
    triggers: [
      ...
      // B-80's shape, found live on 2026-09-06: an operator asked for «аудит
      // скилов» and the hook named only task-pipeline — the description
      // advertised "auditing" while this table carried no audit word at all,
      // so the router text promised ground the hook could not name. The member
      // advertises both phrases as of make-skill 0.27.1; neither is a substring
      // of the refusal «без make-skill», so the refusal stays sayable.
      'skill audit', 'аудит скилов',
    ],
  ...
  copywriting: {
    ...
    triggers: [
      ...
      // Advertised in this skill's own `Triggers -` list and absent from this
      // table: the description promised a phrase the selector could not name.
      'headline', 'заголовок', 'cta', 'кнопка', 'post for x', 'this sounds like ai',
    
      // Advertised by `super-ux` 0.54.0, which corrected «пост для твиттера» — it
      // demanded a literal «для» an operator does not type — and named «текст ошибки».
      // The completeness fixture refused the pin until the table caught up, which is
      // the half of that rule a description edit alone cannot satisfy.
      'пост в твиттер', 'текст ошибки', 'error text',
    ],
  ...
  'agent-sync': {
    ...
    triggers: [
      ...
      // Advertised in this skill's own `Triggers -` list and absent from this
      // table: the description promised a phrase the selector could not name.
      'the submodule is pushed', 'сабмодуль запушен', 'the parent points at the old commit', 'родитель на старом коммите',
    ],
  ...
  'agent-stack': {
    sources: [
      ...
      // Advertised in this skill's own `Triggers -` list and absent from this
      // table: the description promised a phrase the selector could not name.
      'agent system', 'агентная система', 'llm router', 'llm billing', 'биллинг llm',
    ] },
  ...
  'sheleg-dev': {
    sources: [
      ...
      // Advertised in this skill's own `Triggers -` list and absent from this
      // table: the description promised a phrase the selector could not name.
      'add stripe', 'вебхук stripe', 'invoice.paid', 'cancel subscription', 'retention coupon', 'скидка при отмене',
    ] },
...
/**
 * The Russian verbal prefixes `phrasePattern` will step over to reach a stem.
 *
 * Ordered longest-first: regex alternation is first-match, so `пере` must be tried
 * before `пер`-anything and `про` before `по`. The five-letter floor in
 * `phrasePattern` is what keeps this from reaching short stems — `фикс` is four
 * letters, so «зафиксируй результат» stays silent, which it should: recording a
 * result and fixing a defect are different acts that share a shape.
 */
const RU_PREFIX = 'пере|недо|разо|рас|раз|под|при|про|над|об|от|за|из|на|до|со|вы|по';
...
function phrasePattern(phrase) {
  ...
  const words = phrase.trim().toLowerCase().replace(/ё/g, 'е').split(/[\s\-–—]+/);
  const body = words.map(wordPattern).join(gap);
  // A Russian VERBAL PREFIX is part of the same word, and the boundary below used to
  // end the match before the stem was reached. The suffix side has always been
  // tolerant — `wordPattern` returns `stem[а-яё]{0,allowance}` — so «рефакторинг
  // оплаты» routed and «отрефактори модуль оплаты» reached nothing, from one word.
  // Measured across the shipped table: not one prefixed form of any single-word
  // Cyrillic trigger reached its own route. `B-84` recorded this class as settled in
  // v0.85.1 and it was not; the three defects it did settle were all on other axes.
  //
  // Declared, never derived: the list is the prefixes an operator types in front of
  // the verbs this family advertises, longest first because alternation is
  // first-match. `с`, `у` and `в` are deliberately absent — one letter in front of a
  // stem is how a prefix rule starts matching the middles of unrelated words.
  const prefixed = /^[а-яё]/.test(words[0]) && stemRu(words[0]).stem.length >= 5
    ? `(?:${RU_PREFIX})?`
    : '';
  return new RegExp(`(^|[^a-zа-яё0-9])${prefixed}${body}(?![a-zа-яё0-9])`);
}
...
/**
 * A payload that is not a person. Claude Code fires `UserPromptSubmit` for
 * system-generated turns too — background-task notifications, harness events —
 * and on 2026-09-06 every agent-completion notification in one session was
 * "routed": the notification body mentioned `stripe-billing`, so the hook told
 * the operator their own task report "wires money, tracking or sign-in". Worse
 * than the noise: a notification QUOTING a refusal phrase would silence the
 * session's routing without the operator saying anything. The markers are the
 * harness's own framing, printed verbatim at the top of such payloads; a person
 * pasting one mid-prompt loses one routing hint on one turn, which is the cheap
 * side of the trade.
 */
const SYSTEM_TURN_MARKERS = [
  '[SYSTEM NOTIFICATION - NOT USER INPUT]',
  '<task-notification>',
];

function isSystemTurn(prompt) {
  const p = String(prompt || '');
  return SYSTEM_TURN_MARKERS.some((m) => p.includes(m));
}

function optedOut(prompt) {
  // A refusal is the operator's word. A system turn carries nobody's word, so
  // it can neither refuse a route nor spend the session's sticky opt-out.
  if (isSystemTurn(prompt)) return false;
  const p = normalise(prompt);
...
function match(prompt) {
  if (!prompt || isSystemTurn(prompt) || optedOut(prompt)) return [];
  const p = normalise(prompt);
...
/**
 * The typed intent contract (FIX-RT-01). The regex layer above returns route
 * CANDIDATES — a shortlist ranked by term overlap — and candidacy authorizes
 * nothing. What a prompt PERMITS is a separate, typed answer:
 *
 *   { intent, clauses, effects, facets }
 *
 * - `clauses` — the prompt split at sentence and conjunction seams, each with
 *   its own intent (`question` | `audit` | `change` | `other`) and its own
 *   route candidates. Audit and change COEXIST in different clauses: «объясни
 *   гейт и почини в нём баг» keeps the fix, because a question in clause one
 *   never erases the action in clause two.
 * - `effects` — what the prompt authorizes: `write` only when some clause
 *   carries change intent; `report` for audit/question clauses. An audit-only
 *   prompt, RU or EN, authorizes NO write — an audit asks for a verdict, not
 *   a diff, and the router saying "task-pipeline is a candidate" was never an
 *   authorization to edit.
 * - `facets` — the union of route candidates, still candidates.
 */
/**
 * Which route each refusal phrase declines (FIX-RT-01.02). A phrase with no
 * entry here is GENERIC — it opts the whole prompt out, exactly as before.
 * A mapped phrase declines ONE route: «сделай лендинг, но без дизайна» keeps
 * every other clause's candidates, because negating one clause never erases
 * another. The hook-level `optedOut` keeps its one-boolean semantics — this
 * map serves the typed layer only.
 */
const REFUSAL_ROUTE = {
  'без пайплайна': 'task-pipeline', 'no pipeline': 'task-pipeline',
  'без сценариев': 'super-ux', 'no scenarios': 'super-ux',
  'без дизайна': 'sheleg-design', 'no design': 'sheleg-design',
  'без seo': 'seo-llmo', 'no seo': 'seo-llmo',
  'без доков': 'evidence-docs', 'no docs': 'evidence-docs',
  'на словах': 'evidence-docs', 'on my word': 'evidence-docs',
  'без make-skill': 'make-skill', 'no make-skill': 'make-skill',
  'без координации': 'agent-sync', 'no coordination': 'agent-sync',
  'без агентного слоя': 'agent-stack', 'no agent layer': 'agent-stack',
  'без диагностики': 'project-audit', 'no diagnosis': 'project-audit',
  'no telegram': 'telegram-dev', 'без телеграма': 'telegram-dev',
  'no wiring': 'sheleg-dev', 'без обвязки': 'sheleg-dev',
};

/**
 * USING an agent is not BUILDING an agent system (FIX-RT-01.02). The routing
 * block's own boundary — "not every sentence with the word agent" — was not
 * machine-checked: «запусти суб-агента для поиска» matched `agent-stack`'s
 * `sub-agent` trigger and routed a delegation to the builder's doctrine.
 * A clause that matches a USE phrase and no BUILD marker drops `agent-stack`
 * from its candidates; a clause that carries a build marker keeps it.
 */
const AGENT_USE = [
  'запусти сабагента', 'запусти суб-агента', 'запусти агента', 'через сабагента',
  'через суб-агента', 'делегируй агенту', 'сабагентом', 'суб-агентом',
  'use a subagent', 'use a sub-agent', 'spawn a subagent', 'spawn an agent',
  'run a subagent', 'via subagent', 'delegate to an agent', 'launch a subagent',
];
const AGENT_BUILD = [
  'оркестратор', 'orchestrator', 'агентная система', 'agent system',
  'построй агента', 'build an agent', 'multi-agent', 'мультиагентн',
  'агентный слой', 'agent layer', 'граф задач', 'token wallet',
];

/**
 * An explicit skill request is preserved (FIX-RT-01.02): an operator naming an
 * entry point has already chosen the route, and neither the question filter
 * nor the agent-use guard may unchoose it. A refusal of the SAME route still
 * wins — saying the phrase is the decision.
 */
const ENTRY_REQUESTS = {
  '/ux-audit': 'super-ux', '/ux': 'super-ux',
  '/task-pipeline': 'task-pipeline', '/project-audit': 'project-audit',
  '/sheleg-design': 'sheleg-design', '/design-sync': 'sheleg-design',
  '/skill-audit': 'make-skill', '/seo-aeo-audit': 'seo-llmo',
  '/agent-sync': 'agent-sync',
};

function explicitRequests(clause) {
  const out = new Set();
  for (const [entry, route] of Object.entries(ENTRY_REQUESTS)) {
    if (clause.includes(entry)) out.add(route);
  }
  const m = clause.match(/(?:используй|use|через|via)\s+(?:the\s+)?([a-z-]+)(?:\s+skill)?/);
  if (m && ROUTES[m[1]]) out.add(m[1]);
  return [...out];
}

const AUDIT_WORDS = [
  'аудит', 'проверь', 'проверка', 'диагностика', 'ревью',
  'audit', 'review', 'diagnose', 'diagnosis', 'inspect', 'check',
];
const CHANGE_WORDS = [
  'почини', 'исправь', 'фикс', 'сделай', 'добавь', 'внедри', 'доработай',
  'перепиши', 'обнови', 'удали', 'рефактор', 'миграция', 'реализуй',
  'fix', 'implement', 'refactor', 'rewrite', 'update', 'remove', 'build',
  'add', 'create', 'migrate', 'change', 'improve',
];

function splitClauses(prompt) {
  return normalise(prompt)
    .split(/[.;!?\n]|\s+и\s+|\s+а\s+также\s+|\s+and\s+|\s+then\s+|\s+потом\s+/)
    .map((c) => c.trim())
    .filter(Boolean);
}

function clauseIntent(clause) {
  const change = CHANGE_WORDS.some((w) => matches(clause, w));
  // Change outranks the other two INSIDE a clause: «исправь то, что покажет
  // аудит» is an instruction to act, whatever vocabulary rides along.
  if (change) return 'change';
  if (AUDIT_WORDS.some((w) => matches(clause, w))) return 'audit';
  if (isQuestion(clause)) return 'question';
  return 'other';
}
...
function classify(prompt) {
  ...
  // A GENERIC refusal (no route mapping) still silences the whole prompt; a
  // MAPPED refusal declines its one route below and silences nothing else.
  // A QUOTED phrase does neither (FIX-RT-02.02): discussing a refusal is not
  // saying it — `isQuoted` is turnstate's, one implementation for both layers.
  const genericOptOut = REFUSALS.some(
    (r) => !(r in REFUSAL_ROUTE) && refuses(p, r) && !turnstate.isQuoted(low, r));
  if (!prompt || isSystemTurn(prompt) || genericOptOut) {
    return { intent: 'none', clauses: [], effects: { write: false, report: false },
             facets: [], declined: [] };
  }
  const declined = REFUSALS.filter((r) => r in REFUSAL_ROUTE && refuses(p, r)
      && !turnstate.isQuoted(low, r))
    .map((r) => REFUSAL_ROUTE[r]);
  const clauses = splitClauses(prompt).map((text) => {
    const explicit = explicitRequests(text).filter((r) => !declined.includes(r));
    let routes = Object.keys(ROUTES).filter((name) =>
      ROUTES[name].triggers.some((tr) => matches(text, tr)));
    if (routes.includes('agent-stack')
        && AGENT_USE.some((u) => matches(text, u))
        && !AGENT_BUILD.some((b) => matches(text, b))
        && !explicit.includes('agent-stack')) {
      routes = routes.filter((r) => r !== 'agent-stack');
    }
    routes = [...new Set(routes.concat(explicit))]
      .filter((r) => !declined.includes(r));
    return { text, intent: clauseIntent(text), routes, explicit };
  });
  const intents = new Set(clauses.map((c) => c.intent).filter((i) => i !== 'other'));
  const effects = {
    write: clauses.some((c) => c.intent === 'change'),
    report: clauses.some((c) => c.intent === 'audit' || c.intent === 'question'),
  };
  const facets = [...new Set(clauses.flatMap((c) => c.routes))];
  const intent = intents.size > 1 ? 'mixed' : (intents.size ? [...intents][0] : 'other');
  return { intent, clauses, effects, facets, declined: [...new Set(declined)] };
}

/**
 * The waiver LIFECYCLE (FIX-RT-02.02): one turn in, the carried state out.
 *
 * A refusal the USER says (not quoted, not a system turn) becomes a SESSION
 * waiver — it survives every following turn until explicitly revoked with
 * «верни маршрут <route>» / "restore route <route>", and a revoke restores
 * ONLY the route it names. The whole-session opt-out stays a separate
 * deliberate act (`optedOut`), which no revoke here clears. The returned
 * classification is the turn's, narrowed by the session's standing waivers —
 * narrowed, never erased: every un-waived route keeps its candidacy.
 */
const REVOKE_RE = /(?:верни маршрут|restore route)\s+([a-z][a-z0-9-]*)/g;

function applyTurn(state, prompt) {
  let s = state || {};
  const c0 = classify(prompt);
  if (isSystemTurn(prompt)) return { state: s, classification: c0 };
  const raw = String(prompt || '').toLowerCase();
  for (const m of raw.matchAll(REVOKE_RE)) {
    if (ROUTES[m[1]]) s = turnstate.revokeWaiver(s, m[1]);
  }
  const p = normalise(prompt);
  for (const [phrase, route] of Object.entries(REFUSAL_ROUTE)) {
    if (refuses(p, phrase) && !turnstate.isQuoted(raw, phrase)) {
      s = turnstate.recordWaiver(s, route, { source: 'user', scope: 'session' });
    }
  }
  const drop = (r) => turnstate.waived(s, r);
  const classification = {
    ...c0,
    facets: c0.facets.filter((r) => !drop(r)),
    clauses: c0.clauses.map((cl) => ({ ...cl, routes: cl.routes.filter((r) => !drop(r)) })),
    declined: [...new Set([...c0.declined,
      ...Object.keys(s.waivers || {}).filter(drop)])],
  };
  return { state: s, classification };
}

module.exports = {
  match, render, sessionNote, isQuestion, optedOut, matches, isSystemTurn,
  applyTurn,
  stemRu, stemLat, phrasePattern, classify, splitClauses, explicitRequests,
  ROUTES, QUESTION, REFUSALS, AUDIT_WORDS, CHANGE_WORDS,
  REFUSAL_ROUTE, AGENT_USE, AGENT_BUILD, ENTRY_REQUESTS,
};
