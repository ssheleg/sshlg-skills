...
const REFUSALS = [
  ...
  'no scenarios', 'no design', 'as is', 'no brand', 'draft it', 'no seo',
...
const ROUTES = {
  ...
  'project-audit': {
    ...
    // `дизайн`. Both routes firing on «аудит проекта» is correct: the work
    // changes the repository AND its subject is the whole project.
    triggers: [
  ...
  'agent-stack': {
    sources: [
      ...
      { skill: 'agent-stack/agent-harness', triggers: [
        ...
        // so the word is out. A phrase form ('react loop', 'react pattern')
        // cannot replace it yet: a trigger must be a substring the skill's own
        // description advertises, and agent-harness advertises only the bare
        // word — that description edit belongs to agent-stack's own release.
