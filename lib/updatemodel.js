...
function notice(mode, findings, setAuto) {
  ...
  // that reason never changed: nine marketplaces on nine clocks is drift.
  if (setAuto === false) {
    out.push('  The set does not update itself: `update.auto` is off. Run the line above');
    out.push('  when you want the next set — all nine together, which is why there is one');
    out.push('  command and no member argument.');
  } else {
    out.push('  The SET updates itself when you go idle — all nine together, never one on');
