 ...
 * The packs are released and pinned as a set; `skills.json` says which versions belong
...
function notice(mode, findings, setAuto) {
  ...
  out.push('');
  // This paragraph said "Auto-update is OFF for these packs on purpose" while
  // `config list` said `update.auto on` — the launcher telling the operator the
  // opposite of what the pack does, shipped in v1.48.0 and caught by reading its
  // own output back. Two statements of one fact, which is the drift this family
  // files against itself. It describes the ACTUAL state now, and the reason the
  // per-marketplace flag is still left alone survives in both branches, because
  // that reason never changed: one marketplace per member, each on its own
  // clock, is drift.
  if (setAuto === false) {
    out.push('  The set does not update itself: `update.auto` is off. Run the line above');
    out.push('  when you want the next set — all of them together, which is why there is one');
    out.push('  command and no member argument.');
  } else {
    out.push('  The SET updates itself when you go idle — all of them together, never one on');
    out.push('  its own clock, which is why there is one command and no member argument.');
    out.push('  Stop it with `npx sshlg-skills config set update.auto off`.');
  }
  if (mode === 'install') {
    // This said "Nothing checks for you" until 2026-09-10, and that sentence was
    // true and was a cost — the operator had to REMEMBER.
    out.push('  A session-start notice names a newer set when there is one, so nothing');
    out.push('  needs remembering.');
  }
  if (on.length) {
    out.push('');
