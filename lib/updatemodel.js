...
function notice(mode, findings) {
  ...
  if (mode === 'install') {
    ...
    // true and was a cost — the operator had to REMEMBER. A SessionStart notice
    // now says when a newer SET is out, which removes the remembering without
    // touching the reason the per-marketplace flag stays off.
    out.push('  A session-start notice says when a newer SET is out, so nothing needs');
    out.push('  remembering — the command stays yours to run.');
  }
  if (on.length) {
    out.push('');
