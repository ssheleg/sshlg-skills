 ...
 */
/**
 * What a plain copy in a PROJECT costs, and the count is derived from this list.
 *
 * The header said "two consequences" while the body listed three for exactly as long as
 * it took to add one — a restated number, which is the defect this repository catches
 * most often in other people's documents.
 *
 * The third is the one that stops a release rather than dirtying a history, and it is
 * the half a shadow report does not obviously cover: a project's linter DISCOVERS files,
 * and a flat config with no `files` scope reaches whatever the skill vendored. Measured:
 * six skills, seven vendored `.cjs`, and `deploy.sh` refusing to ship on 143 problems —
 * 127 `no-undef`, 15 `no-require-imports` — in files the operator had never seen, in a
 * language the project may not even use.
 */
const PROJECT_HARMS = [
  'it beats the plugin for this project and serves its frozen version;',
  'it is inside a git tree, so `git add -A` commits it into the product;',
  'the project\'s own tooling discovers it. A flat lint config with no\n`files` scope reaches whatever the skill vendored, and the gate goes\nred on code neither the project nor the skill author wrote.',
];
...
function render(rows) {
  ...
  if (proj.length) {
    ...
    out.push(`[sshlg-skills] ${proj.length} shadowing plain ${proj.length === 1 ? 'copy' : 'copies'} ` +
      'in THIS PROJECT\'s .claude/skills/ — ' + PROJECT_HARMS.length + ' consequences, not one:');
    for (const r of proj) out.push(`  ${r.skill} — shadows the plugin from ${r.plugin}${r.at ? ` (${r.at})` : ''}`);
    PROJECT_HARMS.forEach((h, i) => h.split('\n').forEach((line, j) => out.push(
      j === 0 ? `  ${i + 1}. ${line}` : `     ${line}`)));
    ...
    // `.claude/skills/*` and not `.claude/skills/`: git does not descend into an
    // excluded DIRECTORY, so a negation written inside one never fires and the
    // allowlist silently keeps nothing.
    out.push('  The trailing `*` is load-bearing — git does not descend into an excluded');
    out.push('  directory, so a `!` negation written inside one never fires.');
    out.push('  The same pair belongs in the lint and format ignore lists, which are');
    out.push('  usually enumerated by literal directory and are therefore silent about');
    out.push('  the next tree something installs into.');
  }
  return out.join('\n');
}
