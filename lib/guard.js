...
const repogate = require('./repogate.js');
...
/**
 * The path sits immediately after a redirect.
 *
 * Three shapes were missing, and each is an ordinary way to destroy the file:
 *
 * - **A quote.** This is tested against `segment.slice(0, at)`, which ends in `"` or
 *   `'` when the path is quoted — so the `>` was no longer at the end and
 *   `echo x > "$HOME/.claude/CLAUDE.md"` walked past. `spellings()` enumerates
 *   `$HOME/…` and `${HOME}/…` ON PURPOSE, so the author meant to catch that line and a
 *   pair of quotes cancelled it. Quoting a `$HOME` path is standard agent practice.
 * - **A file descriptor.** `[^0-9<>&]` excluded a preceding digit or `&`, so `2>` and
 *   `&>` were treated as not-a-redirect. Both truncate exactly as `>` does.
 * - **`>|`.** The clobber form. `segments()` splits on `|`, so the path landed in a
 *   segment with no redirect in front of it — handled there, not here.
 *
 * Measured 2026-09-01: of ten spellings, three were caught and seven were not.
 * What stays out of scope, deliberately, is a path resolved through `cd` — that needs
 * `payload.cwd` and the resolver `lib/repogate.js` already carries, and it is a change
 * with its own fixtures rather than a regex. It is on the board.
 */
const REDIRECT = /(^|[^<>])(?:\d+|&)?>{1,2}\|?\s*["']?$/;
 ...
 *
 * A `|` immediately after `>` is NOT a pipe — it is the clobber redirect `>|`, and
 * splitting there put the path in a segment with no redirect before it, which is how
 * `echo x >| ~/.claude/CLAUDE.md` walked past the whole module.
 */
...
function decide(payload, home) {
  ...
  if (typeof input.command === 'string') {
    // A HEREDOC BODY FED TO A NON-SHELL IS DATA, NOT A COMMAND.
    //
    // `python3 - <<'EOF' … print('do not touch ~/.claude/CLAUDE.md') … EOF` was flagged
    // as a write: the body carries the path beside a word this guard reads as a write
    // verb, and nothing in it ever runs as shell. Measured 2026-09-05 — the python case
    // fired, the node case did not, which is the shape of an accident rather than a
    // rule. `lib/hygiene.js` has carried the treatment since `B-59` and this module
    // never received it (`B-136`, second spelling).
    //
    // IMPORTED, not copied: a second heredoc parser is exactly what this family's
    // copied-mechanism guard exists to catch, and the two would drift on the first fix
    // applied to one of them. What survives the strip still runs — the text before the
    // `<<` keeps its redirect, so `cat > FILE <<'EOF'` is still a write, and a body fed
    // to `bash` or `sh` is kept whole because it genuinely executes.
    const segs = segments(executablePart(input.command));

    // THE SHELL MOVES, AND THE PATH GETS SHORTER.
    //
    // `cd ~/.claude && echo x > CLAUDE.md` writes the same file as
    // `echo x > ~/.claude/CLAUDE.md`, and only the first walked past: `spellings()`
    // enumerates absolute and `$HOME` forms, none of which appear in a command that has
    // already moved. The segments run in order, so the cwd is tracked across them and
    // the relative spelling is offered alongside the others.
    //
    // The resolver is `lib/repogate.js`'s, exported rather than copied — this family
    // keeps a guard against copied mechanisms, and a second `cd` parser is exactly what
    // it exists to catch.
    //
    // Widening this is cheaper than it was: since v1.20.0 the hook answers no permission
    // decision on the happy path, so an over-catch costs an unnecessary copy rather than
    // the operator's own prompt.
    let cwd = typeof p.cwd === 'string' && p.cwd ? p.cwd : home;
    for (const seg of segs) {
      const dest = repogate.cdTarget(seg);
      if (dest) {
        const joined = repogate.joinPath(cwd, dest);
        cwd = joined.startsWith('~')
          ? path.join(home, joined.slice(1))
          : path.resolve(cwd, joined);
        continue;
      }
      for (const file of list) {
        for (const spelling of spellings(file, home)) {
          if (writesTo(seg, spelling)) return file;
        }
        // The file as this segment's shell would name it — only when it is UNDER the
        // cwd, because `../../.claude/CLAUDE.md` is not a spelling anyone types.
        const rel = path.relative(cwd, file);
        if (rel && !rel.startsWith('..') && !path.isAbsolute(rel) && writesTo(seg, rel)) {
          return file;
        }
      }
    }
