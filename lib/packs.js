...
const LANES = [
  ...
  {
    id: 'a11y',
    asks: 'whether it can be used at all',
    owner: null,
    fallback: 'accesslint',
    refusal: 'no a11y',
    // B-140 asked the family to choose: a router takes this lane, or the family says
    // permanently that it delegates it. It delegates it, and the reason is measured
    // rather than preferred — an eleventh router was tried and refused.
    delegated: true,
    standing: 'The family DELEGATES accessibility, permanently and on purpose. An '
      + 'eleventh router was the obvious answer and was measured to be the wrong one: '
      + 'the routing eval on 2026-08-31 stopped a block change because every arm '
      + 'widened routing on the silence probes — a new router costs precision across '
      + 'ten existing ones. What the family owns instead is saying so: this lane '
      + 'prints its owner as absent, `test/packs_test.js` asserts the null so it '
      + 'cannot be filled quietly, and the fallback names a third party rather than '
      + 'implying cover. A delegation stated is not the same as a gap ignored.',
  },
...
const PACKS = {
  design: {
    ...
    entries: [
      {
        ...
        // The generic `why` is safe to print unverified — it describes the
        // upstream's purpose, not its current file. The SPECIFIC ban is a CLAIM
        // resolved against the reached SKILL.md (FIX-VD-07.01): the catalog
        // ascribed 'bans Inter/Roboto/Arial' to a skill whose reachable file on
        // this machine carries no such ban, so a specific is printed only when
        // the resolved descriptor confirms it — otherwise it is an unverified,
        // generic upstream claim, never stated as fact.
        why: 'refuses the default look before a line is written — pushes one committed aesthetic direction',
        claims: [{ text: 'bans Inter / Roboto / Arial', needles: ['Inter', 'Roboto', 'Arial'] }],
      },
      {
      ...
      },
      {
        ...
        lane: 'figma',
        // `claude-plugins-official` alone is a marketplace name, not an address —
        // `--check` reported it "unreachable" as if the network had failed, when
        // nothing had been asked (family audit 2026-09-06). The owner-qualified
        // form is the repository the marketplace serves from.
    ...
    // --- connected surfaces, considered rather than merely absent (B-86) ---------
    // Measured 2026-08-17 by grepping every shipped skill for each surface name:
    // `refero`, `mobbin`, `lazyweb` and `figma` are named by `sheleg-design` and
    // `super-ux`; `chrome-devtools` by `seo-aeo-audit` and `task-pipeline`; these three
    // by nobody. The cost of that silence is that an agent cannot tell *considered and
    // refused* from *never looked at* — the same failure `B-06`'s close had to write
    // down for the memberless members. So each one says which it is.
    surfaces: [
      {
        id: 'pencil',
        what: 'an MCP editor for `.pen` design files — a second design editor beside Figma',
        verdict: 'refused',
        reason: 'the pack has a Figma seam because tokens cross it in both directions — '
          + 'variables in, code out — and that seam is what makes the handoff checkable. '
          + 'A second editor with no token bridge would add a place designs live and no '
          + 'way to verify what arrives. Revisit when `.pen` exposes variables the '
          + '`tokens` lane can read.',
      },
      {
        id: 'google_lens / google_images',
        what: 'uncurated image search, reachable through `searchapi`, which `seo-aeo-audit` already uses',
        verdict: 'refused',
        reason: 'the three curated corpora — Mobbin, Refero, Lazyweb — answer *what has '
          + 'shipped and worked*, with the product, the flow and often the experiment '
          + 'attached. An image search answers *what exists*. Feeding the reference lane '
          + 'from it would make a moodboard out of a corpus the pack chose deliberately, '
          + 'and the difference is the whole reason the reference lane names three sources '
          + 'rather than a search box.',
      },
      {
        id: 'higsfield',
        what: 'image, video and audio generation',
        verdict: 'refused for the reference lane, available to the operator',
        reason: 'generating a moodboard is not finding a reference: the first invents '
          + 'agreement with itself, the second is evidence that something shipped. No lane '
          + 'in this pack asks for generated imagery, so it is named here rather than '
          + 'wired — an operator reaching for it is doing something the pack has no '
          + 'opinion about, which is different from something it refused.',
      },
    ],
    declined: [
      {
...
function assertSurface(pack, x, seen) {
  const at = `${pack}/surfaces/${x && x.id ? x.id : '<unnamed>'}`;
  if (!x || !x.id) throw new Error(`${at}: a surface row needs an id`);
  if (seen.has(x.id)) throw new Error(`${at}: id already used by another row`);
  seen.add(x.id);
  if (!x.what) throw new Error(`${at}: needs one line saying what the surface IS`);
  if (!x.verdict) throw new Error(`${at}: needs a verdict — considered and refused reads differently from never looked at`);
  // The reason is the whole row. A verdict with no reason records that somebody had an
  // opinion, which is what the silence already recorded.
  if (!x.reason || x.reason.length < 80) {
    throw new Error(`${at}: a verdict with no measured reason is the silence it replaced`);
  }
  if (x.install) throw new Error(`${at}: carries an install command — a surface row is a decision, not a recommendation`);
}
...
function assertPack(pack) {
  ...
  for (const x of pack.surfaces || []) assertSurface(pack.id, x, surfaceIds);
  // A lane that declares itself delegated must SAY so in full. `owner: null` alone is
  // a gap; `delegated` without a standing statement is a gap wearing a decision's name.
  for (const l of pack.lanes || []) {
    if (!l.delegated) continue;
    if (l.owner) throw new Error(`${pack.id}/lanes/${l.id}: delegated and owned at once`);
    if (!l.standing || l.standing.length < 120) {
      throw new Error(`${pack.id}/lanes/${l.id}: declares a delegation with no standing statement — the family has to say what it delegates and why, or the lane is still a gap`);
    }
  }
  return pack;
}
...
function renderLanes(pack, present) {
  ...
  // A lane the family DELEGATES prints its standing statement here, once. `GAP` above
  // says nobody owns it; that is a measurement. Whether nobody owns it on purpose is a
  // decision, and a reader who cannot tell them apart re-opens the question every time.
  for (const lane of pack.lanes) {
    if (!lane.delegated || !lane.standing) continue;
    lines.push('');
    lines.push(`    ${lane.id} — DELEGATED, and this is the standing decision:`);
    lines.push(...wrap(lane.standing, 72, '      '));
  }
...
function report(pack, skills, opts) {
  ...
  // Surfaces the pack has CONSIDERED — the answer to «this tool is connected and no
  // pack names it». Silence cannot distinguish refused from never-looked-at, and an
  // agent reading the silence re-opens the question every session (B-86).
  if (!o.lane && (pack.surfaces || []).length) {
    lines.push('');
    lines.push(`  connected surfaces this pack has considered (${pack.surfaces.length})`);
    for (const x of pack.surfaces) {
      lines.push('');
      lines.push(`    ${x.id}   ${x.verdict}`);
      lines.push(...wrap(x.what, 74, '      what: '));
      lines.push(...wrap(x.reason, 74, '      '));
    }
  }
...
function checkReport(results) {
  ...
  for (const r of rows) {
    let verdict;
    // A source that is not an `owner/repo` address never went near the network,
    // and printing it as `unreachable` blamed the network for a declaration —
    // `figma` wore that verdict for a release (family audit 2026-09-06). The
    // declaration defect counts as `bad`: it is this repository's to edit.
    if (r.error && /not a repository address/i.test(r.error)) {
      verdict = `UNDECLARED (${r.error})`; undeclared += 1; bad += 1;
    } else if (r.error) { verdict = `unreachable (${r.error})`; unreachable += 1; }
  ...
  lines.push('  repository — re-run it before editing anything. A row reading');
  lines.push('  `UNDECLARED` is the opposite: the declaration itself, no network involved.');
  return { text: lines.join('\n'), bad, unreachable, undeclared };
}
 ...
 */
/**
 * Which platform a candidate actually serves (FIX-VD-03.03). Derived from the
 * lane, because the estate has no iOS/Android-native tool: the `mobile` lane's
 * candidate is React Native / Expo, every other design lane is web.
 */
function platformOf(entry) {
  return entry && entry.lane === 'mobile' ? 'react-native' : 'web';
}

const NATIVE_PLATFORMS = new Set(['ios-native', 'android-native']);

/**
 * Select a pack's candidates for a HOST/PLATFORM target, not just the web
 * default (FIX-VD-03.03). A native-iOS or native-Android request has NO
 * matching candidate in this estate, and the resolver says so — it does NOT
 * hand back the React Native skill or a web tool and let it read as native
 * readiness (a SwiftUI brief is not answered with Expo, the same rule
 * sheleg-design's platform/renderer split states). Returns:
 *   { platform, matches: [...], native_unsupported: bool, note }
 */
function selectForPlatform(pack, platform) {
  const want = platform || 'web';
  const entries = (pack && pack.entries) || [];
  const matches = entries.filter((e) => platformOf(e) === want);
  if (NATIVE_PLATFORMS.has(want) && !matches.length) {
    return {
      platform: want,
      matches: [],
      native_unsupported: true,
      note: `no candidate serves ${want} in this estate — the mobile lane covers `
        + 'React Native/Expo only, and no tool covers native iOS/Android. Do NOT '
        + 'present the RN or a web candidate as native readiness; the native '
        + 'implementation is unowned and must be declared so.',
    };
  }
  return { platform: want, matches, native_unsupported: false, note: null };
}
...
/**
 * Resolve an entry's content-specific CLAIMS against the skill actually reachable
 * on this machine (FIX-VD-07.01). The catalog's generic `why` describes the
 * upstream's purpose and prints as-is; a `claims` assertion about the file's
 * CONTENT must be confirmed against the resolved descriptor, or it is reported
 * as unverified rather than printed as fact.
 *
 * Pure: `reachable` is the list the caller read off disk — [{id, path, source,
 * text}]. Returns the resolved file (path + sha256 digest, so a route trace
 * records the exact version it read), every provider found (two implementations
 * of one name are BOTH shown), and the claims split into confirmed / unverified.
 */
function resolveEntry(entry, reachable) {
  const crypto = require('crypto');
  const provides = new Set((entry && entry.provides) || []);
  const providers = (Array.isArray(reachable) ? reachable : [])
    .filter((r) => r && provides.has(r.id));
  const claims = (entry && entry.claims) || [];
  if (!providers.length) {
    // Nothing reached: every specific is unverified, and the entry is a generic
    // upstream description until a file is present to confirm it against.
    return {
      resolved: null,
      providers: [],
      confirmed: [],
      unverified: claims.slice(),
      generic: true,
    };
  }
  const resolved = providers[0]; // precedence: first reachable; both are reported
  ...
  for (const c of claims) {
    const needles = (c.needles && c.needles.length) ? c.needles : [c.text];
    const ok = needles.every((n) => lower.indexOf(String(n).toLowerCase()) !== -1);
    (ok ? confirmed : unverified).push(c);
  }
  return {
    resolved: { id: resolved.id, path: resolved.path, source: resolved.source, digest },
    providers: providers.map((p) => ({ path: p.path, source: p.source })),
    confirmed,
    unverified,
    generic: false,
  };
...
module.exports = {
  ...
  platformOf, selectForPlatform, NATIVE_PLATFORMS, resolveEntry,
};
