...
function save(opts) {
  ...
  // The stamp resolves to the second, and an agent edits faster than that. Two
  // copies of one file inside the same second would land on the same name, and
  // the second would overwrite the first — losing exactly the content the first
  // copy existed to preserve. Found by the end-to-end fixture, which had meant to
  // test something else entirely.
  const base = nameFor(file, home, opts.stamp || stamp(new Date()));
  let name = base;
  for (let n = 2; fs.existsSync(path.join(dir, name)); n += 1) name = `${base}-${n}`;
...
/**
 * The most recent copy of `file`, or `null`.
 *
 * A snapshot is taken by one hook and read by another — `PreToolUse` copies the
 * config `obsidian-wiki setup` is about to truncate, `PostToolUse` puts back what
 * it dropped — so the two need a way to name the same copy without passing state
 * between processes. The stamp sorts lexicographically the way it sorts in time,
 * which is why the last name is the newest file and no `stat` is needed.
 */
function latest(opts) {
  const dir = opts.dir || path.join(opts.home, ...DIR);
  if (!fs.existsSync(dir)) return null;
  const key = keyFor(opts.file, opts.home);
  const mine = fs.readdirSync(dir).filter((n) => n.startsWith(key + '.')).sort();
...
module.exports = { save, guard, latest, stamp, keyFor, nameFor, toPrune, DIR, KEEP };
