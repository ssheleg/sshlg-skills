...
//: A pipe into an interpreter that reads its program from stdin.
const STDIN_PROGRAM = /\|\s*(?:python3?|node|ruby|perl)\s+-(?=\s|$)|\|\s*(?:sh|bash|zsh)\s+-s(?=\s|$)/;
const HEREDOC = /<<-?\s*['"]?\w+['"]?/;
...
function inspect(command) {
  ...
  // ONE STDIN, TWO PASSENGERS. `producer | python3 - <<'PY'` sends the secret
  // AND the program down the same channel; the pipe wins, the interpreter
  // parses the connection string as source, and SyntaxError prints the line it
  // could not parse — password included. Measured on the operator's own agent,
  // 2026-09-14 (project-observatory DEC-0227). The producer's shape does not
  // matter: any pipe into a program that reads its code from stdin, with a
  // heredoc beside it, is this mistake.
  if (STDIN_PROGRAM.test(command) && HEREDOC.test(command)) {
    return { rule: 'secret-into-stdin-program', why:
      'This pipes into a program that reads its CODE from stdin (`python3 -`, `node -`, '
      + '`sh -s`) while a heredoc also feeds stdin. The pipe wins: the interpreter parses '
      + 'the piped value as source, cannot, and prints the offending line in its error — '
      + 'that is how a DATABASE_URL reached a transcript on 2026-09-14.\n'
      + 'Put the program in a file so stdin carries one thing:  … | python3 probe.py\n'
      + 'Or hand the value over as a variable with the output scrubbed:\n'
      + '  … | ~/DATA/project-observatory/tools/use_secret.py pipe NAME -- python3 probe.py' + TAIL };
  }
