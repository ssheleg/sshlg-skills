...
function readSkills(home) {
  ...
  // FIX-RT-03.01 — a skill is DISCOVERED by its directory/symlink, but only
  // CALLABLE when it resolves to a live SKILL.md. A broken symlink, a same-named
  // foreign target, or a bare directory is discovered-not-callable, and the
  // active-host roster must not present it as available. Each entry also carries
  // its PROVENANCE (plugin spec or plain path) and a content DIGEST, so identity
  // is namespace+provenance+bytes, never a bare id.
  const crypto = require('crypto');
  const skillMd = (dir) => path.join(dir, 'SKILL.md');
  const digestOf = (dir) => {
    try { return crypto.createHash('sha256')
      .update(fs.readFileSync(skillMd(dir))).digest('hex').slice(0, 16); }
    catch (e) { return null; }
  };
  const callableAt = (dir) => {
    try {
      // realpath resolves a symlink; a broken link throws → not callable.
      const real = fs.realpathSync(dir);
      return fs.existsSync(path.join(real, 'SKILL.md'));
    } catch (e) { return false; }
  };
  const add = (plugin, dir, id, provenance, namespace) => {
    const key = `${plugin}/${id}`;
    if (seen.has(key)) return;
    seen.add(key);
    const callable = callableAt(dir);
    out.push({ plugin, id, description: describe(dir),
               callable, provenance, namespace,
               digest: callable ? digestOf(dir) : null });
  };
  ...
  for (const [spec, on] of Object.entries(enabled)) {
    ...
    // A directory under `skills/` is not a skill unless it carries a `SKILL.md` —
    // and `super-ux` ships `skills/references/` as a shared reference directory for
    // its seven real skills, so the roster the routing block MANDATES an agent read
    // before substantial work handed back a skill that does not exist, from the
    // family's own flagship pack, with a blank description because `describe()`
    // found no front matter to read. It also inflated the headline count.
    // The plain-copy scan below already has this problem in a different form and is
    // left alone here: a symlink in `~/.claude/skills` IS the skill, and the shadow
    // detector needs to see it whether or not the target resolves.
    for (const d of names) {
      if (!d.isDirectory()) continue;
      const dir = path.join(base, d.name);
      if (!fs.existsSync(path.join(dir, 'SKILL.md'))) continue;
      add(spec, dir, d.name, `plugin:${spec}`, 'claude:plugin');
    }
  }
  ...
  for (const d of names) {
    ...
    // Kept even when not callable (the shadow detector needs to SEE a broken or
    // foreign symlink) — but `callable` marks it so the roster can exclude it.
    add('(plain ~/.claude/skills)', path.join(plain, d.name), d.name,
        `plain:${path.join(plain, d.name)}`, 'claude:plain');
  }
